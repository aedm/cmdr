...
Frontend counterpart: `apps/desktop/src/lib/ai/CLAUDE.md` for the AI status surface, model picker, BYOK key
management, and the inline suggestion UI.
...
- **`client.rs`**: `genai`-backed chat client. `AiBackend` is a struct bundling a long-lived `genai::Client` with a model name; built via `AiBackend::local(port)` or `AiBackend::remote(api_key, base_url, model)`, both `pub(in crate::ai)` so production code outside `ai/` can only get a backend from `resolve_backend` (tests elsewhere use the `#[cfg(test)]` `remote_for_tests` / `local_for_tests`). For `remote`, the model name picks the adapter via the pure `remote_model_iden`: `claude-*` → Anthropic native, `gemini-*` → Gemini native, `gpt-*`/`o1*`/`o3*`/`o4*`/`chatgpt-*` → OpenAI (with `genai`'s `gpt-5*`/`*-codex`/`*-pro` → Responses-API auto-routing), and EVERYTHING ELSE is forced onto the OpenAI chat-completions adapter via the `openai::` namespace. That last rule is load-bearing: `genai` falls back to its **Ollama** adapter for unrecognized model names, so a bare `openai/gpt-oss-20b` (Groq), `deepseek-chat`, or `google/gemma-…:free` (OpenRouter) would POST to Ollama's `/api/chat` against an OpenAI endpoint and 404 — every BYOK provider except Anthropic/Gemini speaks OpenAI chat-completions. Auto-omits `temperature`/`top_p` for the OpenAI Responses adapter and for chat-completions reasoning models (`o1*`, `o3*`, `o4*`, `chatgpt-*`, `gpt-5*` defense-in-depth) and substitutes `ReasoningEffort::Low`. Local backend forces the OpenAI adapter via a `ServiceTargetResolver` pinning endpoint to `http://127.0.0.1:<port>/v1/`. Exposes `chat_completion` (full response), `chat_completion_with_empty_retry` (retries once with 4× the token budget on `EmptyResponse` — the translate commands use this), and `chat_completion_stream` (returns a `BoxStream<Result<String, AiError>>` of content chunks; reasoning/thought-signature/tool-call chunks filtered out). `AiError` is typed by HTTP status via the pure `ai_error_for_status` (401/403 → `AuthFailed`, 404 → `NotFound`, 429 → `RateLimited`, else `ServerError`); a `None` `first_text()` → `EmptyResponse`. `NotFound` is the wrong-model-id case — a typo, or one the provider decommissioned — and also a base URL with the wrong path, since 404 can't tell them apart; it carries no toast of its own (it maps to the `ServerError` translate kind), and exists so the provider smokes can name a stale pin by variant rather than by matching prose. `map_genai_error` reaches the classifier from BOTH shapes `genai` uses: `WebAdapterCall`/`WebModelCall` for a one-shot call, and `WebStream` holding a boxed `HttpError` for a stream. The stream shape is easy to miss and load-bearing: while it was unhandled, every streaming failure fell to the catch-all, so a rejected key showed as a generic server error on the path the agent and folder suggestions use. The variant's detail string comes from the pure `provider_error_detail`, which extracts the JSON body's `error.message` (the human sentence OpenAI-style providers all use), falling back to the raw body capped at 400 chars — display only (the toast's secondary line, the Ask Cmdr error bubble), never control flow. Two `pub(crate)` seam methods serve the agent LLM (`agent/llm/genai_impl.rs`), which needs a multipart tool loop the prompt-only helpers can't express: `resolve_adapter` (the resolved `AdapterKind`, for the agent's per-provider reasoning posture) and `exec_chat_stream_request` (runs a caller-built `ChatRequest` through this backend's adapter routing + `adjust_for_model`, returning the raw genai stream). `map_genai_error` is `pub(crate)` so the agent maps `AiError` onto its own typed error from one shared status classifier. **`client.rs` is also the LLM-call-logging tap**: each dispatch function logs the outgoing `ChatRequest` and the response through `llm_log/CLAUDE.md` when the backend carries a context (`with_log_context`) and the `logLlmCalls` setting is on — the one seam both the agent and the legacy helpers pass through. Details, fidelity, privacy: `llm_log/DETAILS.md`.
- **The real-API provider smokes** (`client_real_{groq,fireworks,anthropic,openai,gemini}_test.rs`): `#[ignore]`-gated live calls through `AiBackend::remote`, one file per provider. They catch adapter-routing / auth / parse regressions the wiremock tests can't (it's what caught the Ollama-fallback bug above), plus the thing only a live call can see: a provider retiring the model we name. Each has a `<provider>-smoke` check lane (Go runner) that resolves the key from env or the sops `secret` helper and runs it with `--run-ignored only`, self-skipping without one. All are CI-only, so locally they run on demand (`pnpm check groq-smoke`); CI: `slow-checks.yml` has a step each. Mechanics and the CI-only rationale: `scripts/check/checks/DETAILS.md`.
  - **Why five and not one.** Groq alone only ever notices Groq's schedule. The set covers each distinct code path once: Groq and Fireworks the forced `openai::` namespace (Fireworks additionally the `accounts/…/models/…` id shape, which must survive namespacing and `Url::join` intact); Anthropic and Gemini the two non-OpenAI wire formats we ship; OpenAI all three of its legs (chat-completions, the Responses API, and the `o*` chat-completions reasoning models that reject `temperature`) — so those three tests are deliberately NOT interchangeable, and `client.rs`'s `the_two_openai_smoke_models_cover_different_branches` fails if a swap collapses them. Adding a sixth OpenAI-compatible host would buy nothing; every remaining gap is a protocol, not a vendor.
  - **Gemini's lane carries a THIRD outcome, because its free tier flaps.** The identical request answered 200, then `503 UNAVAILABLE`, then a **zero-byte-body 404**, then sustained 503s, inside a few minutes (verified 2026-09-04, repeated `curl` against `v1/models/gemini-3.5-flash-lite:generateContent`). So the lane can't read every failure as a decommission, and it can't quietly pass either. It splits them structurally, never by wording:
    - Google filled in its JSON error envelope (`{"error": {"code", "message", "status"}}`) on a 404 ⇒ it routed the request and is telling us the model is withdrawn ⇒ hard failure through `expect_ok`, naming `GEMINI` and the model-list URL. (verified 2026-09-04: the withdrawn `gemini-2.5-flash-lite` answers exactly that, pointing at `gemini-3.5-flash-lite`.)
    - Zero-byte body, 503, or a dead connection ⇒ Google never routed it ⇒ four attempts with 5/10/20 s backoff, then `smoke_providers::report_inconclusive`, which the lane turns into a **WARN**. Rationale in `scripts/check/checks/desktop-rust-provider-smoke.go`: a red lane on Google's load trains everyone to ignore the nightly, and a green one is the silent-skip trap again.
    - A bodyless 404 ALSO means "Google doesn't serve this path", so a wrong `base_url` reads identically to an outage (verified 2026-09-04: `/v1beta/openai/models/…:generateContent` and a version-less `/models/…:generateContent` both answer it while `v1` is healthy). Both suspects live in the same constant, so a lane that stays inconclusive night after night means "read `GEMINI`", not "wait it out".
  - **The inconclusive report travels through a file**, not stdout: nextest sets `success-output = "never"`, so a passing test's `println!` is discarded exactly when it matters. The lane creates a temp path, passes it as `CMDR_SMOKE_STATUS_FILE` (the same env var for every provider, so any lane can start reporting one), and warns if the file comes back non-empty. Run by hand with no such file, `report_inconclusive` panics rather than passing vacuously.
  - **Gemini's smoke must stay on the NATIVE adapter**, or the lane covers a path Groq already covers. `remote_model_iden` leaves `gemini-*` un-namespaced so `genai` POSTs `{base}models/{model}:generateContent`; two tests pin it, `smoke_gemini_routes_to_the_native_adapter` (asserts `AdapterKind::Gemini` from `resolve_adapter`, no network) and, keylessly, `client_unit_test::smoke_provider_models_route_to_their_intended_adapters`.
  - **Gotcha: a streaming failure arrives as a CHUNK, not as an error from `chat_completion_stream`.** `genai` hands back an `Ok` stream before the response status is known, so a chunk unwrapped outside the retry loop skips the triage above and reports every Gemini outage as a decommission — which is what an earlier shape of `smoke_gemini_stream` did against a deliberately wrong base URL. `stream_one` opens AND drains inside one retryable unit for that reason.
  - **`.config/nextest.toml` gives `test(ai::client_real_gemini_)` 150 s**, ahead of the general 60 s `ai::client_real_` override (nextest: first match per setting wins). The retries deliberately spend ~35 s waiting, which the shorter cap would SIGKILL mid-backoff — turning "Google was busy" into the red lane the retry exists to prevent.
...
Centralized in `manager::resolve_backend(app) -> BackendResolution` (the pure decision is `resolve_backend_inner`, split out of the global lock so it's unit-testable, mirroring `compute_ai_status`):
- `Off`: provider is `"off"`.
- `NoCloudConsent`: provider is `"cloud"` and the user hasn't allowed cloud AI. Checked before the key and endpoint.
...
The canonical home for how "Allow cloud AI" works. Nothing reaches a cloud AI service (folder suggestions, search and
selection translation, MCP `ai_search`, Ask Cmdr's rail and wake turns, the connection check) until the user turns
the switch on. Local AI needs none: nothing leaves the Mac.

- **One chokepoint.** Every LLM backend comes from `manager::resolve_backend(app)`, which reads consent only when
  `ai.provider` is `cloud` and answers `NoCloudConsent` before looking at the key or endpoint. Taking the `AppHandle`
...
- **Versioned.** The record stores the accepted `CLOUD_AI_CONSENT_VERSION`. Bump it whenever the `ai.cloudConsent.*`
  disclosure copy changes materially (widening what any feature sends is exactly that), so everyone re-consents.
- **Stored in `main.db`'s `meta` table** (`cloud_ai_consent_version`, `cloud_ai_consent_at`; `../agent/store/consent.rs`),
  because that store is durable, migrated, transactional, and inspectable, and the held-"no" machinery already lived
  around it. Ask Cmdr's old opt-in keys stay there, read-only by construction, read only by the one-time mapping onto
  `askCmdr.enabled`; they grant nothing.
- **A refused "no" is held.** When `main.db` refuses the revoke, the frontend writes `ai.cloudConsentRevokePending` to
  `settings.json` and calls `cloud_ai_consent_revoke_pending_changed`. `RevokePending::load` reads it fresh on every
  check, so the "no" holds at once and the retry only makes the store catch up.
- **Turning it off stops cloud AI immediately.** `revoke_cloud_ai_consent` clears the record, then cancels every
  in-flight Ask Cmdr turn (`agent::chat::cancel::cancel_all`, rail and wake) and every folder-suggestion stream
  (`stream_registry::cancel_all`). A held "no" does the same. Translate calls are one-shot and short: they finish, and
  the next one refuses. `ai.provider` is untouched.
- **The connection check is gated too.** `check_ai_connection` carries no user data, but it reaches the service, so
  without consent it answers `cloud_consent_missing: true` and sends nothing. It's gated whatever `ai.provider` says:
  the only thing it probes is a cloud endpoint being set up, and onboarding can probe before `configure_ai` has
  pushed the provider.
...
- **Adding a local model**: add the GGUF to `AVAILABLE_MODELS` in `mod.rs` with those two fields, and set `DEFAULT_MODEL_ID` if it should be the default. Get the exact file size the entry needs with `curl -sIL "<url>" | grep -i content-length` — the download is only marked complete after that size verifies, so a wrong number means a truncated model never launches llama-server.
