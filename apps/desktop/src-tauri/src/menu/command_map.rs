...
/// IDs for the menus themselves, as opposed to the items inside them. Submenus fire no events, so
...
pub const FILE_NEW_FILE_ID: &str = "file_new_file";
...
/// Menu item ID for "Suggested ops" (opens the review dialog for what the agent has
/// suggested, under the View menu, next to "Operation log").
pub const SUGGESTED_OPS_ID: &str = "suggested_ops";
...
pub fn menu_id_to_command(menu_id: &str) -> Option<(&'static str, CommandScope)> {
    match menu_id {
        ...
        SUGGESTED_OPS_ID => Some(("suggestedOps.show", CommandScope::App)),
        ...
        FILE_NEW_FILE_ID => Some(("file.newFile", CommandScope::FileScoped)),
...
pub fn command_id_to_menu_id(command_id: &str) -> Option<&'static str> {
    match command_id {
        ...
        "suggestedOps.show" => Some(SUGGESTED_OPS_ID),
        ...
        "file.newFile" => Some(FILE_NEW_FILE_ID),
