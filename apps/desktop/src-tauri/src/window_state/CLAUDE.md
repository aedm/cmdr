...
- `mod.rs`: Tauri wiring. Load + `init`, `restore` (apply saved geometry, placement only), `track` (Moved/Resized/Close
...
- **❌ Never call `show()` from here.** `restore()` does placement only. The frontend shows the main window through the
  `show_main_window` command, which orders it to the *back* under E2E so test runs don't steal focus, so a bare `show()`
  here would make every E2E run steal it. The only backend show is `crate::main_window_show`'s E2E-exempt fallback. The
