...
**Every local write lands via temp + rename** (`overwrite::stage_and_land_file`). Steps: write the bytes to `dest.cmdr-tmp-<uuid>` (a sibling, so the landing rename is same-directory and therefore atomic); if an existing entry is being replaced, rename dest → `dest.cmdr-temp-<uuid>` (the aside); rename temp → dest; delete the aside. The original is intact until the landing rename completes, and the destination name never holds a partial at any observable moment. The same pattern covers file→folder overwrites (existing dest folder renamed aside, then the source file lands at the original path). A folder→file overwrite has two shapes, because the aside can only go once the new content is complete: `safe_overwrite_dir` where the closure finishes the job (the symlink branch, the move engines), and `displace_with_directory` where it doesn't — the copy's parent-creation site, whose subtree lands leaf by leaf, hands the aside to the `CopyTransaction` and lets commit, a reversal, or a failure answer for it (a failure keeps it as a ` (recovered)` sibling, since the half-filled folder keeps the name). `transfer/DETAILS.md` § "Cross-type overwrites" holds the why.
...
**Replacing one kind of entry with another needs a person's consent, per SHAPE.** `Overwrite`, `OverwriteSmaller`, and `OverwriteOlder` all say "the entry at the destination is stale, put the source there", which is an answer about two FILES. Across types the act is a different one — replacing a folder with a file discards the folder, replacing a file with a folder discards the file — so `conflict::resolution_for_clash` reduces any of the three to `Skip` unless somebody was actually asked, and the item reports as an ordinary skip. All three engines call it, so the rule is one rule: local (`conflict.rs`), cross-volume (`transfer/volume/conflict.rs`, both the up-front lookup and the Stop arm's under-the-lock re-check), and in-archive (`archive_edit/conflicts.rs`, both `ConflictMode` arms).

What counts as consent, and what doesn't:

- **Refused**: the configured `conflict_resolution` (nobody picking a policy in the transfer dialog was shown either kind), and a same-kind apply-to-all carry reaching across types. The latch alone can't defend this — a config policy that IS `Overwrite` walks straight past it.
- **Honoured**: a plain `Overwrite` answered on a Stop prompt for THAT shape, **and the "* all" carry it latched**. The dialog names both types (`build_conflict_event`'s `source_is_directory` / `destination_is_directory`) and relabels its apply-to-all button for the act ("Overwrite folders with files"). Refusing the carry makes that button a trap: the first item replaces, every one after it is skipped, and the local engine's per-file skip isn't even visible in the toast.
- **Refused whoever asked**: `OverwriteSmaller` / `OverwriteOlder`, including straight off the prompt. The dialog offers both on a cross-type prompt, but neither label names the act and neither can ask its own question here — a directory's `len()` is its own inode's, so every ordinary file looks "bigger" and "Overwrite all smaller" deleted destination folders wholesale. The Stop arm has to run its answer through `answered_resolution_for_clash` BEFORE `reduce_conditional_resolution`, or the reduction does that comparison against a directory.
...
- **`resolve_source_volume(volume_id, first_path)`** routes a batch that a ROUTE serves to that read-only volume, and returns which route it was as an `Option<RoutedKind>`. Two routes qualify: an archive-inner batch goes to its `ArchiveVolume`, and a path inside a repo's virtual `.git` trees goes to the repo's `GitPortalVolume`. One `source_volume_id` per batch means no straddle risk, so the first path decides. A cheap string pre-filter (`volume::manager::path_routes_over_its_parent`) keeps a plain path off the parent-aware resolve entirely, and it is deliberately narrow at the edges: the `.zip` FILE itself is a plain file copied through its parent volume, and a `.git` path counts only while the portal is switched on.
- **A routed source is the whole point of the routed shape.** Neither namespace has an inode, so a source left on its parent volume takes `copy_between_volumes`'s both-local fast path against a path `std::fs` can't stat: the scan finds nothing, the preview caches zero files, and the transfer dialog sits on "Verifying before copy" until it ends in "Couldn't finish copying". Routed, both read with `open_read_stream` and walk with `scan_for_copy` on the cross-volume engine. The same generalization runs in `commands/file_system/write_ops.rs::scan_preview_source_volume`, which sizes the transfer before it starts.
- **`RoutedKind` rather than a bool, because the two routes part ways on a MOVE.** An archive can delete its source entries after the extract lands (the compound move-out op), a snapshot cannot: a file in a branch snapshot has no inode to remove. So `start_volume_move` refuses a `GitPortal` source with the typed `ReadOnlyDevice`, named for the source volume, before the destination is even resolved. A `GitPortal` DESTINATION is refused the same way in both `start_volume_copy` and `start_volume_move`: every mutation on that volume answers `NotSupported`, so a drop onto a snapshot would otherwise start and die on its first write. A `.zip` destination is NOT this case; it routes to the archive-edit driver, which really does write.
...
- **A skip decided MID-FLIGHT reports `Skipped`**, via `scan::FileVerdict` returned from `copy_single_item`. This is the one the pre-flight can't see: the local engine resolves per-file conflicts as it walks (a cross-type refusal, a Skip on a clash inside a folder, a source that already IS its destination), long after the bulk-skip pass has run. `copy_single_item` used to return `Ok(())` for both outcomes, so the driver counted every one of them as a transfer — the operation reported "Copied 2 files." having copied one, `files_skipped` stayed 0, and the `had_conflicts` analytics proxy (`files_skipped > 0`) missed the whole class.

**A source is `Done` if ANY of its files landed.** The skip is per file, so one refused child inside a folder doesn't make the folder a skip; `SourceItemTracker` tracks "landed something" per top-level source and only reports `Skipped` for one that landed nothing.

⚠️ **`FileVerdict` says what the POLICY decided, never what is on disk**, and a destructive act must not read it. The cross-FS move's phase-4 delete set is decided by the LEDGER (`transaction.created_files()` growing across the call), because that records what actually got written; the verdict feeds reporting only, where being wrong costs a wrong label instead of a file. Delete passes `CarriedOut` unconditionally: it has no Skip, so reaching the tracker means the entry is unlinked.
...
**Decision**: Copy and move are durable before they report complete: per-file `sync_data` (fdatasync) in chunked copy, plus an end-of-op pass that syncs the data the strategy didn't and fsyncs every directory that gained an entry. A cross-FS move deletes its sources only when that pass answers `Ok`. Delete and trash don't sync at all.
**Why**: "Complete" must mean "durable on disk," not "buffered in the OS page cache." Without it, a user who copies to a USB stick / SD card and ejects (or the machine sleeps) right after "Copy finished" loses the file — and on a move it's gone from both source and dest. The flush is targeted, not a whole-machine `libc::sync()`: that global sync also stalled unrelated apps (AGENTS.md principle #5). The mechanism: (1) `transfer/chunked_copy.rs` calls `dst_file.sync_data()` per file, so each file is durable as it completes — a crash mid-batch on a long transfer leaves earlier files safe. (2) Before emitting `write-complete`, `durability::flush_created_destinations` emits a `Flushing`-phase progress event, `fdatasync`s every created file the strategy didn't already make durable (it reuses `CopyTransaction.created_files` and `created_dirs`, no parallel dest-tracking), and `fsync`s every directory that gained an entry. It answers a `Result`: the cross-FS move gates its source delete on it, the copy only logs a failure. Same-FS move (pure rename) writes no data at all, so it takes the sibling pass `flush_touched_directories`: one `fsync` per DIRECTORY the renames touched, and none on the moved files, which would cost an `F_FULLFSYNC` device barrier each for bytes nothing wrote. The rules (which errnos count as "not supported", the staged-to-landed path mapping, what a failure keeps) and the measurements live in `transfer/DETAILS.md` § "Durability (flush before reporting complete)". Pinned by `transfer/copy_tests.rs::local_copy_emits_flushing_phase_before_complete` and `transfer/move_op/move_progress_tests.rs::cross_fs_local_move_emits_flushing_phase_before_complete`; FE label by `TransferProgressDialog.flushing.test.ts`. **Cross-volume copy/move landing on a local disk** (MTP → Local, SMB → Local, USB import) doesn't go through this local-FS engine — it flows through `LocalPosixVolume::write_from_stream`, which keeps the same promise by `sync_data`-ing each file (plus a best-effort parent-dir fsync for the directory entry) before it returns, so each file is durable as it completes. That path doesn't yet emit the `Flushing` UI phase (the volume copy/move handlers don't call `flush_created_destinations`); a follow-up could route them through the end-of-op pass for UI consistency, but the per-file `sync_data` already makes them durable.

**Decision**: `TauriEventSink::emit_error` logs the whole typed error at **WARN**, and that is the only place a failed write operation's technical reason is written down.
**Why**: The variant travels to the frontend, which raises the user-facing error and shows the detail string under the dialog's collapsed "Technical details". Nothing wrote it to the log, so an error-report bundle recorded the frontend's `Trash error: io_error` and nothing else — ERR-BPAPM (2026-09-08) lost the OS's own words for a `trashItemAtURL` refusal, unrecoverably. The all-items-failed branch of `trash_files_with_progress` was the specific hole (its partial-failure sibling below it does log per-item details), but the gap was every operation's, so the line sits at the one shared emit site rather than in each engine. **WARN, deliberately, not `log_error!`**: the frontend already raises the same failure at error level, and `log_error!` trips the auto-dispatcher, whose Flow B window opens at two errors within 60 s — a second error-level line would let one user-visible failure auto-send a report on its own. The file target is always Debug, so a warn still reaches every bundle. It logs `{:?}` of the variant rather than a hand-written sentence, because the variant already carries the path and the message in named fields and Debug can't drift from them. The frontend side of the same lesson is `cmdr/no-unrendered-log-fields` (`src/lib/logging/DETAILS.md`).
