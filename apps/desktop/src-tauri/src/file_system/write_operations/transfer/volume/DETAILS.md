...
- **`remove_tree(volume, path, why: TreeRemoval)`** — the recursive sweep, with the first-child-failure reporting
  ...
  one variant is the only authorization that exists: `UserChoseOverwriteAcrossTypes` (`conflict.rs`, a cross-type
  Overwrite the user picked). A move's SOURCE is never a tree removal, into a zip included: it goes through the ledger
...
The `remove_tree` caller (cross-type Overwrite in `conflict.rs`) and the into-zip move's source sweep only log, and they
