...
- **`conflict.rs` DECIDES; two thin siblings act and answer.** The resolver keeps the policy (`resolve_volume_conflict`,
  `apply_volume_conflict_resolution`, the conditional reduction, `size_of_whatever_is_at`). `finalize.rs` holds the
...
- **`remove_tree(volume, path, preserve, why: TreeRemoval)`** — the recursive sweep, carrying the `preserve` set and the
  ...
  (`conflict.rs`, a cross-type Overwrite the user picked), `MoveSourceAfterDestinationLanded` (`move_cross.rs`, after
...
**A probe that can't ANSWER fails the item, and is never read as "the name is free."** `conflict.rs::size_of_whatever_is_at` is the one rule for all four top-level pre-check sites (`copy_serial.rs`, `move_cross.rs`, `move_same.rs`, and the concurrent driver's `copy_concurrent_source.rs::existing_dest_entry`, which carries the same shape over a `FileEntry`): only `VolumeError::NotFound` means free, and every other refusal — `ConnectionTimeout`, `DeviceSessionReset`, `PermissionDenied` — becomes a failure of THAT item at the DESTINATION path. Nothing is written for it. A stat that fails is what a flaky share or a phone mid-session-reset looks like, and folding it into "nothing is there" runs no resolver, consults no Skip/Stop policy, and lets the landing clear whatever the probe was asked about — a silent overwrite under a policy that promised the opposite. The driver's `FetchFut` carries the `Result` so the shape is unrepresentable rather than only written down. ❌ No retry here: per-file retry is `retry.rs`'s, inside `stream_pipe_file`, and a second layer would multiply the wait on a dead link. Pinned by `dest_precheck_failure_tests.rs` (one cell per site) and `transfer_driver_async_tests.rs::async_driver_fails_the_item_whose_destination_probe_refuses`. `merge.rs::what_the_destination_holds` is the same discipline one level down, for a merge child.
...
`merge_level` answers each child's "is this name taken?" from the level listing through the SAME `DestNameIndex`, so `Report.docx` arriving where the user keeps `report.docx` — or an NFD name against a stored NFC one — reaches the resolver at depth 5 exactly as it does at the top. `what_the_destination_holds` maps the three answers: `Present` carries the listed entry into `resolve_merge_child` (and into the dir-vs-dir decision), `Absent` is a fresh write, `Unknown` costs one `get_metadata` of the child's destination path, which the backend resolves the way it will resolve the write.
...
**Cross-volume move source-delete is recursive.** `move_volumes_with_progress` in `move_cross.rs` deletes the source via `cleanup.rs::remove_tree`, authorized as `MoveSourceAfterDestinationLanded`, when the source is a directory. The `Volume::delete` contract is "file or *empty* directory": `LocalPosixVolume::delete` calls `std::fs::remove_dir` which fails ENOTEMPTY, so deleting a populated source directory after a cross-volume copy must walk the tree. Regression coverage: the `remove_tree_*` tests in `cleanup_tests.rs`. The original failure mode (data at both source and dest, FE shows generic `io_error`) traced back to this; the SMB collision that surfaced on retry was just the second-order symptom.
