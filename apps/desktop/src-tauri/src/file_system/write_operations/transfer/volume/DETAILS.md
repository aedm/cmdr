...
- **`remove_tree(volume, path, why: TreeRemoval)`** — the recursive sweep, with the first-child-failure reporting
  ...
  two variants are the only authorizations that exist: `UserChoseOverwriteAcrossTypes` (`conflict.rs`, a cross-type
  Overwrite the user picked) and `ArchiveMoveSourceAfterCommit` (`archive_edit/copy_into.rs`, remote originals after
  the rewrite durably commits). A cross-volume move's SOURCE is not a tree removal: it goes through the ledger sweep in
  `source_sweep.rs` (§ "The move invariant"). `remove_tree` logs the variant
