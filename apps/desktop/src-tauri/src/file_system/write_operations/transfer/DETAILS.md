...
**What the refusal looks like.** An `AlreadyExists` from the three transfer sites rides out as the engine's ordinary `IoError` and fails the operation. `../rename.rs` maps it to `MutationError::AlreadyExists { name }` (❌ never a sentence — the frontend words it in ten locales) exactly as its volume branch does. On a filesystem with no atomic flag `rename_no_replace` degrades to check-then-rename, which is racy but still strictly better than an unconditional clobber. Pinned by `move_op/move_race_tests.rs` and `rename/tests.rs::{rename_managed_local_conflict_without_force_is_transparent, rename_managed_local_force_overwrites}`.
