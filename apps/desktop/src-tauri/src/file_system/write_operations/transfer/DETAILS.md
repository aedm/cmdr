...
- **`move_op/` splits the local move by engine**: `mod.rs` picks between them on `is_same_filesystem` and owns what
  ...
  callers at once), `same_fs.rs` holds the `rename(2)` engine, `cross_fs.rs` the five-phase staging engine, and `source_sweep.rs` its
  Phase 4 (the ledger-driven source delete). Both
  engine tests all attach to `mod.rs`, so each can reach both engines: `move_op/move_op_tests.rs` (engine behavior),
  `move_op/move_source_sweep_tests.rs` (what Phase 4 may remove), `move_op/safety_matrix_tests.rs` (one data-safety
  invariant swept over both engines × every resolution), and `move_op/move_journal_tests.rs` (the journal + reversal
  full loop), over the rig in `move_op/test_support.rs`.
...
**A local copy's ledger has one reversal and one net.** `copy/rollback.rs::rollback_with_progress` is the Rollback button's, emitting `write-progress` with `phase: RollingBack`, rechecking each entry, and reading the intent between deletions so the user can stop the reversal itself. `CopyTransaction`'s `Drop` is the panic net, and the only unconditional sweep. (`reversal.rs::reverse_copy_transaction` is the same walk without the events, kept `#[cfg(test)]` so the recheck rules can be exercised on a bare ledger.)

**A FAILURE keeps every file that landed**, on both `copy/mod.rs` error arms (`PostLoopIntent::Failed`, and a `create_scanned_dirs_at_destination` that fails): commit the ledger, emit `write-error`, keep going no further. The only thing cleaned is the partial, which `overwrite::stage_and_land_file` already removed on its way out — a local write never leaves one at a real filename. ❌ Never auto-reverse here: an Overwrite's original is gone the moment the new bytes take its name (`stage_and_land_file` step 4 deletes the aside), so deleting the replacement leaves the user with NEITHER copy, and a mid-batch read error would cost them every file the copy had already replaced. This matches the volume engine's error arm (`volume/copy.rs`, "keep completed files, clean up partial files"). The user still has the full reversal from history, where they choose it with the whole picture in front of them. Pinned by `copy/copy_failure_tests.rs`.
...
- **Volume** (`volume/merge_ctx.rs::CreatedPaths`): the byte count the leaf copier reports, which the journal row for the same leaf already carries. `copied_paths` and `CreatedPaths.files` are the same entries, so the size rides through instead of being dropped at the hand-off.
...
`reversal.rs` holds the decision, and every reversal a person can observe routes through it: `copy/rollback.rs::rollback_with_progress` (the Rollback button), `move_op/mod.rs::MoveTransaction::rollback`, and `volume/cleanup.rs::volume_rollback_with_progress`.
...
**The move path carries a SECOND guard**, the one the history engine pins alongside the recheck: the restore is non-destructive. `fs::rename` replaces its target silently and there is no backup to put back, so an original source somebody has since filled skips with `RestoreTargetOccupied`. **The stat that reports that is advisory; the refusal lives in the rename** (`move_op/mod.rs::restore_rename`), which goes through `overwrite::rename_no_replace` (what it guarantees and where it degrades: `file_system/volume/backends/DETAILS.md` § "Local renames are atomic-exclusive"). A stat can only say what was there when it looked, so a check-then-`fs::rename` pair would still destroy a file created in the window between them, silently and with nothing to restore from. The carve-out is the case-only self-collision — a case-insensitive filesystem folds `dog.jpg` and `DOG.jpg` onto one entry, so a case-only move finds its own destination sitting at the source. Same node id ⇒ the same entry ⇒ the rename back is a case correction. One local filesystem means `(dev, ino)` settles it exactly, with no path folding needed.

**A merge's reversal recreates the folders the merge removed.** `merge_move_directory` renames a level's children across and then removes the emptied source directory, so a rollback that only replayed renames would hit `ENOENT` on every child whose parent was gone, leave the whole first folder merged into the destination, and still report it undone. `MoveTransaction` is therefore a ledger of STEPS (`MoveStep::Renamed` / `MoveStep::RemovedSourceDir`), not of renames, and the ORDER is what makes it work: children are recorded before their level's removal and the recursion unwinds deepest-first, so popping the stack newest-first recreates the outermost directory, then the inner one, then lands each child. Nothing has to reason about nesting. A recreation is deliberately NOT tallied: a folder that can't come back makes every child under it report `Skipped` on its own, which is the honest partial-rollback report, and counting the folder as well would either double-count the failure or inflate "put back N items" with something the user never counted as an item. Pinned by `move_op/move_reversal_tests.rs::a_rollback_after_a_merge_recreates_the_source_folders_and_brings_the_children_home`. (The JOURNAL-driven reversal still refuses a merged move outright, for a different reason: `operation_log/DETAILS.md` § "Why a directory merge isn't reversible". This is the in-flight Cancel-with-rollback, which has the per-child ledger the journal doesn't.)
...
full bar with the entire source removal still ahead. The sweep is real, unbounded work — however many files and
directories the move carried, over however large a tree — so it owes a bar of its own.

The denominator is the TOP-LEVEL sources, not the leaves Phase 2 counted: the sweep's own unit of work is one source's
ledger, and a leaf-granular bar would re-count what Phase 2 already showed. A source SKIPPED at Phase 3 counts
...
**The cross-FS source sweep deletes a LEDGER, never a tree** (`move_op/source_sweep.rs`). Phase 2 records every source file it actually staged; Phase 4 removes exactly that list, then the scanned directories under each source, deepest first, each with `fs::remove_dir`. A directory still holding anything answers `ENOTEMPTY`, which the sweep treats as success: the directory stays and so does whatever kept it alive. ❌ `remove_dir_all` has no place here — it acts on what is on disk NOW, and the gap between the scan (taken when the transfer dialog opened, possibly a queue wait ago) and the sweep is long enough for a download to finish, a sync client to land a file, or an editor to save. Removing the tree by identity destroys those files without ever copying them; deleting the ledger is what makes the move invariant ("never delete the source if the destination isn't fully in place") true of the whole source rather than only of the scanned set.

Three kinds of original stay behind, and the sweep tells them apart. A **Skip** takes its original out of the ledger, in either phase: `copy_single_item` records nothing for a leaf it walked past (a clash inside the staging area, a type-mismatch parent Skip, a same-file no-op), and Phase 3's Skips arrive as `skipped_source_paths` (whole top-level source for a single-file / type-mismatch Skip; per-child paths remapped from the staging prefix back to the source prefix for a directory merge), which filters the ledger before the sweep plans. Those are what the user asked for, so they are not reported as news. Anything else still standing **appeared during the move**: the sweep counts it against `ScanResult`'s own paths (one count per unknown subtree, since that's the item a person recognizes in the pane) and hands `AppearedDuringMove { item_count, changed_count, folder_name, folder_count }` to the completion event, which the frontend words. The operation still completes as a success.

An original **saved over after its copy started** stays too. Phase 2 stamps each file (`SourceStamp`: size, mtime, and `(dev, ino)`, via `symlink_metadata`) right BEFORE `copy_single_item` reads it, so a write that lands during the copy counts as well as one after it, and the stamp rides the ledger as `LandedOriginal`. The sweep stats each original again just before its `remove_file`; any difference, or a stamp that couldn't be taken, keeps the file, and its directories stay through their own `ENOTEMPTY`. The count goes out as `changed_count`, named by the folder holding it (a folder source's own name, a file source's parent's). Without it an app saving into the source between the copy and the sweep (the whole rest of the copy, plus the flush) lost that save: the destination held what the copy read and the delete took the only copy of the new bytes. **Decision/Why an mtime here**: the in-flight ledgers refuse one (§ "What the in-flight ledgers record") because a destination filesystem can store a coarser clock than the source, but this compares the SAME file on the SAME filesystem twice, so truncation hits both stats alike and can't invent a change. A same-size save inside one coarse tick is the gap mtime can't see, which is what the node covers for rename-into-place saves. The remaining window is the two syscalls between the recheck and the unlink; POSIX has no compare-and-unlink. Pinned by `move_source_drift_tests.rs`. The cross-volume engine follows the same ledger rule with a listing-based stamp, and the two share the tally (`left_in_source.rs`): `volume/DETAILS.md` § "Cross-volume move source-delete removes a LEDGER".

A source that survives the sweep, for any of these reasons, ends on `SourceItemOutcome::Skipped` with `source_removed: false`, read off `symlink_metadata` after the sweep rather than off what the sweep intended — `source_removed` is the vanished-path contract the search-snapshot purge acts on. The same-FS path (`move_with_rename`) needs none of this: it renames originals directly, and anything it didn't carry just leaves the source dir non-empty. Pinned by `move_source_sweep_tests.rs` and `move_op_tests.rs::{cross_fs_move_skip_preserves_source_and_dest, cross_fs_move_dir_merge_skip_child_preserves_source_child}`.

**Empty directories land via the scanned-dirs pass (`copy/scanned_dirs.rs::create_scanned_dirs_at_destination`).** The per-file loop creates directories only as FILE parents, so an empty directory — or a branch holding nothing but empty directories — has no file to hang its creation on and used to complete "successfully" while never arriving (and on a cross-FS move, Phase 4 then deleted the source: the empty dir was destroyed without ever landing). The pass runs over `ScanResult.dirs` on the local copy's Completed arm and after the move's staging loop (destination = the staging dir, so empty dirs ride the normal Phase-3 rename + cleanup). Mapping mirrors `FileInfo::dest_path`; created dirs are recorded for rollback. Data-safety: a dest path that already holds anything (dir = merge, file = type clash) is left untouched — an empty source dir never replaces user data. Nor does the pass build under a FILE: a dir whose nearest existing ancestor is a file sits inside a folder→file clash that ended in Skip, and `create_dir_all` there would fail with ENOTDIR and take the whole operation down over a clash it has already settled (pinned by `cross_type_policy_tests.rs::a_skipped_folder_with_subfolders_finishes_and_leaves_the_file`). The pass parks per directory like every other loop (`../DETAILS.md` § "Pause / resume"), which matters because it starts AFTER the per-file loop: "Paused" is already on screen by then, so an ungated pass kept building the destination skeleton. Pinned by `copy_tests.rs::{copy_creates_empty_directory_at_destination, copy_creates_nested_empty_directories, copy_empty_directory_does_not_clobber_same_named_dest_file, a_paused_copy_stops_landing_scanned_dirs_until_it_resumes}` and `move_op_tests.rs::cross_fs_move_preserves_empty_directories`. The volume (MTP/SMB) pipeline doesn't share the hole — `copy_directory_streaming` creates each dir before walking its children.
...
**Volume-side Rename reserves the name on local-FS dests (TOCTOU guard).** `volume/naming.rs::find_unique_volume_name` is the volume sibling of `conflict::find_unique_name`. The candidates it walks come from `conflict::NameCandidates` (see `../DETAILS.md`), the same sequence the local-FS namer walks, so a volume dest numbers identically and `photo (1).jpg` continues to `photo (2).jpg`; this function owns only the reservation. When the destination volume is local-FS-backed (`dest_volume.local_path().is_some()`), it reserves the chosen `name (N)` with an `O_CREAT|O_EXCL` placeholder at the resolved local path (via `resolve_local_path`, which mirrors `LocalPosixVolume::resolve`) and escalates on `AlreadyExists`. The streaming writer (`LocalPosixVolume::write_from_stream` → `std::fs::File::create`) truncates the placeholder, so the new bytes land on it cleanly — there's no `COPYFILE_EXCL` in the volume copy path to refuse the overwrite. For backends without exclusive-create semantics (MTP / SMB / InMemory, `local_path()` is `None`) it falls back to the non-atomic `exists()` probe, with a documented narrow residual window. Pinned by `volume/naming_tests.rs::{local_fs_rename_reserves_the_chosen_name_on_disk, local_fs_rename_keeps_extension_in_the_right_place, non_local_dest_does_not_reserve_a_placeholder}`. **The reservation is the CALLER's to take back.** `find_unique_volume_name` answers a `ClaimedName` that says whether it put a file on disk, because on disk that zero-byte placeholder is indistinguishable from an empty file the copy produced, and only the caller knows whether its write ever happened. The top-level drivers already covered it (the resolved write path is their in-flight partial, cleaned by the post-loop); a DEEP merge child did not, so a leaf that gave up (an unreadable source, a cancel between chunks) left a `clash (1).txt` that `CreatedPaths` never learned about — `merge.rs::copy_leaf` records on SUCCESS — and no rollback or sweep could reach. `copy_leaf` calls `take_back_reservation` on its failure path, which deletes the placeholder ONLY while it is still empty: a size means somebody else wrote there, and deleting a stranger's bytes to tidy up our own reservation would be the worse bug. Two shapes never reach that path: a leaf whose FUTURE is dropped (the driver's cancel-drain deadline, or a failing sibling's `LeafPool::submit` returning before the leaf is ever polled) runs no cleanup at all, and the plan-mode path (`sequential_extract.rs`) reserves every clashing name at plan time and streams in a later pass that can stop partway. So the namer also records each placeholder on the op (`ClaimedNames::hold_placeholder`, on `state.claimed_names`), every landed write releases it (`stream_pipe_file`'s two `Ok` returns, the sequential data pass's commit), and the copy and cross-volume move post-loops take back whatever is left, whatever the ending (`naming.rs::take_back_unfilled_reservations`, same empty-only rule). ❗ A write path that can land on a reserved name owes the release: an EMPTY source that landed looks exactly like the placeholder, and on a move it is the only copy. A same-volume move clears its own placeholder before renaming onto the name and releases it there; it runs no sweep. Pinned by `volume/merge_placeholder_tests.rs` (including `an_empty_file_that_lands_on_its_reservation_is_kept`) and `volume/strategy_sequential_tests.rs::a_failed_sequential_extract_takes_back_every_unfilled_reservation`.

**Cross-type overwrites (file↔folder).** Reachable only from a plain Overwrite a person answered on a Stop prompt for that SHAPE, its "* all" carry included; a policy nobody was asked about refuses across types (`../DETAILS.md` § "Replacing one kind of entry with another needs a person's consent, per SHAPE"). Move and the copy's symlink branch route it through `overwrite::safe_overwrite_dir`; the copy's folder→file case has its own shape, below:
- **Local copy `copy_single_item`'s parent-creation site (folder→file).** The source tree wants a directory where a file is. `overwrite::displace_with_directory` renames the file aside as `<name>.cmdr-temp-<uuid>` and creates the directory, and the CopyTransaction takes BOTH: the created dir as an ordinary `record_dir` row, and the aside as a `record_displaced` entry. `safe_overwrite_dir` is the wrong shape here and used to be the bug (finding 13): it drops the aside the instant its closure returns `Ok`, but this landing isn't finished then — the directory is empty and the subtree arrives leaf by leaf over the rest of the copy. The aside went the moment `create_dir_all` returned, the directory was never recorded (the `if !parent.exists()` walk below it sees the dir it just made), and a cancel or a failing first leaf left an empty `X/` where an `X` file had been, with nothing to restore and nothing a rollback could even remove. Now the transaction answers for it, and there are THREE answers. `commit()` discards the aside: what replaced it is staying and complete (the Completed path). Every reversal (`copy/rollback.rs::rollback_with_progress` and the `Drop` net) puts it back — LAST, after the files are removed and the dirs pruned, so the name is free when the original comes home; a `rename_no_replace`, so anything else standing there is left alone. A FAILURE or a plain STOP is the third, and neither of the first two fits it: both KEEP every file that landed (`PostLoopIntent::Failed`, finding 6), so the half-filled folder keeps the name too — discarding the aside would delete the user's only copy of that file, and restoring it has nowhere to go. `commit_keeping_displaced_aside` renames it to a ` (recovered)` sibling instead (`DisplacedEntry::keep_as_recovered_sibling`, over the shared `unique_name::recovered_sibling` the volume engine's finalize rescue uses, continuing the house ` (N)` series when that name is taken), and `copy/mod.rs::fail_keeping_displaced_aside` wraps the failure in `WriteOperationError::OriginalsKeptAside` so the dialog names every path (a stop goes through `stop_keeping_displaced_aside`, which can only log it: the cancel event has no field for it yet). The wrapped `cause` is the failure that actually stopped the copy, so a full disk still reads as a full disk. Pinned by `copy/copy_failure_tests.rs::{a_failed_folder_over_file_overwrite_keeps_the_users_file_beside_the_folder, a_failed_folder_over_file_overwrite_names_the_recovered_path}` and `folder_over_file_rollback_tests.rs::a_stopped_folder_over_file_overwrite_keeps_the_users_file_beside_the_folder`. The local MOVE engine has no such stretch: a same-FS move replaces across types with one rename inside `safe_overwrite_dir`, and a cross-FS move copies into a fresh staging tree (nothing to displace) and lands each item the same way. Only `blocking` itself is created here, so any deeper level stays with the ordinary create-and-record walk and reaches the ledger too. A restore is deliberately not tallied: the person counted the items they asked to copy, and one of their own files returning isn't one. Pinned by `folder_over_file_rollback_tests.rs` and `write_operations/tests.rs::{displace_with_directory_keeps_the_file_until_the_transaction_answers, a_committed_transaction_drops_what_it_displaced}`.
- The symlink branch's file→folder overwrite goes through `safe_overwrite_dir`, correctly: a symlink is complete the moment its closure creates it.
...
**Decision.** Every local move engine treats a symlink as a LEAF: it renames the link itself, and never walks through one. So a link whose target is a directory does NOT merge with a same-named directory at the destination; that pair is a type mismatch, and it goes through `resolve_conflict` like any other (prompt on Stop, leave both sides on Skip, land aside as a link on Rename). The same holds mirrored: a real directory in the source meeting a LINK at the destination is a type mismatch too.

**Why.** `Path::is_dir()` is `fs::metadata`-based and follows links, so the obvious "both sides are directories ⇒ merge" test says yes for a link. The merge then `read_dir`s the LINK, which lists the target's entries, and renames each one out of a folder the user never selected (`~/work/app/node_modules -> ~/shared/node_modules` against an `~/archive/app/node_modules/` that already exists empties the shared folder). Afterwards `remove_dir` on the link fails `ENOTDIR` and the link survives pointing at what it lost. The destination side is the mirror: renaming into `<dest>/<link>` lands the user's files wherever the link points, outside the destination they chose.

**Where the one question lives, locally.** `validation.rs::is_real_directory` (`symlink_metadata` + `is_dir`), asked at all four sites that could descend: `same_fs.rs`'s top-level dir-vs-dir test, `move_op/mod.rs::merge_move_directory`'s per-child test, `cross_fs.rs`'s phase-3 test against the STAGED tree (a staged link is still a link), and `move_op/mod.rs::move_resolved_into_place`, whose source/dest type split decides between a plain rename and the `safe_overwrite_dir` type-mismatch dance. `conflict.rs`'s `is_file_to_folder` classification asks the same question of the entries with `symlink_metadata`, so the apply-to-all latch buckets a link as the leaf it is. Pinned by `move_op/move_symlink_tests.rs`.

**And on the volume side.** The same-volume rename-merge can't stat: it decides from the LISTING, and `listing/reading.rs` reports a link-to-directory as `is_directory: true` (`metadata.is_dir() || target_is_dir`). `FileEntry::is_symlink` rides along for exactly this, so `rename_merge.rs::merges_as_a_directory` (`is_directory && !is_symlink`) is the volume twin of `is_real_directory`, asked at both recursion sites (the per-child dest-map hit and `late_detected_collision`'s case-folded one), at the `source_is_directory_hint` the resolver is given, and at `apply_child_decision`'s entry-type branch. Nothing in the merge probes with `Volume::is_directory` to decide whether to descend, because that follows links too. Pinned by `volume/rename_merge_symlink_tests.rs`.

**The TOP-level source of a volume move follows the same rule** (#140). `preflight.rs::top_level_move_hints` derives each hint with `merges_as_a_directory` off the parent listing, and trusts a TransferDialog preview for FILES only: a copy scan answers "does this stream as a folder", which a link to one does, so a preview can't tell the two apart. Where there's no hint, and wherever a PATH (not a listed entry) needs typing, the question is `Volume::entry_kind` (`File` / `Directory` / `Symlink`, a link reported as the link), ❌ never `Volume::is_directory`: whether that follows a link is the backend's business, and an ADB phone's or an SFTP server's does. So `move_same.rs`'s source and destination probes, `conflict.rs::resolve_dest_is_directory`, and `cleanup.rs::remove_tree` all ask `entry_kind`. `remove_tree` is the one that loses data when it gets this wrong: every cross-volume move's source sweep runs through it, and recursing through a link deletes the target's files.

**A cross-volume transfer materializes a link; it doesn't keep it.** The streaming engines (`copy_serial.rs`, `copy_concurrent*.rs`, `move_cross.rs`) take the scan's answer, so a link to a folder copies as the folder it points at, at every depth, and meets a same-named destination folder as a merge. That's deliberate: SMB, MTP, and WebDAV destinations can't hold a link, and it destroys nothing, because a copy never touches the source and a move's sweep then removes the LINK (above). Local-to-local transfers never get here; the local engines copy a link as a link.

Pinned by `volume/rename_merge_symlink_tests.rs` (the top-level cells, including the one arriving through a preview) and `volume/link_following_backend_tests.rs`, which runs the move engines on a `LocalPosixVolume` stripped of its `lstat` overrides so `is_directory` follows links the way a phone's does.

### A move's landings refuse an occupied name

**Decision.** Every rename a move makes onto a name it believes is FREE goes through `move_op/mod.rs::rename_onto_free_name`, which is `overwrite::rename_no_replace` (`renamex_np(RENAME_EXCL)` on macOS, `renameat2(RENAME_NOREPLACE)` on Linux). Three sites: `same_fs`'s top-level no-conflict branch, `merge_move_directory`'s per-child one, and `cross_fs`'s staging-to-final rename. The inline rename (`../rename.rs`, the local branch) does the same, keeping its `force` bypass for the caller who asked to replace and for a case-only self-rename that folds onto its own target.

**Why.** "Is this name taken?" and "take it" are two operations, and a plain POSIX `rename` replaces its target without a word. A file another process creates in that window is destroyed with no prompt, no conflict entry, and no backup: the exact loss the conflict machinery exists to prevent, reached by walking around it. The window is one syscall wide, so this is rare rather than theoretical — and the fix is the primitive every copy landing and bulk rename already used. Refusing fails the item and leaves both copies on disk, which the user can still sort out.

**What the refusal looks like.** An `AlreadyExists` from the three transfer sites rides out as the engine's ordinary `IoError` and fails the operation. `../rename.rs` maps it to `MutationError::AlreadyExists { name }` (❌ never a sentence — the frontend words it in ten locales) exactly as its volume branch does. Pinned by `move_op/move_race_tests.rs` and `rename/tests.rs::{rename_managed_local_conflict_without_force_is_transparent, rename_managed_local_force_overwrites}`.
...
2. **End-of-op pass.** Before `write-complete`, every handler that wrote bytes calls `durability::flush_created_destinations` (the same-FS move, which wrote none, takes the directory-only sibling below). It emits a `Flushing`-phase `write-progress` event (the FE renders **"Writing the last piece..."** instead of a bar frozen at 100%; see the FE doc), then:
   - `fdatasync`s every created file not in `already_synced` (chunked-synced files and clonefile/reflink dests, which are CoW-shared and moot to flush). A symlink has no data of its own.
   - `fsync`s every directory that gained an entry: the parent of every created file AND of every directory in `created_dirs`, each distinct directory once, whatever `already_synced` says. A directory that only received a `mkdir` is covered by its parent's fsync.
   - Answers `Result<(), FlushFailure { path, errno }>`, trying every path before answering the first failure. A directory fsync failing with the "this filesystem doesn't do that" class (`ENOTSUP`, `EOPNOTSUPP`, `EINVAL`, `ENOSYS`, `ENOTTY`, matched as raw `libc` values that differ between macOS and Linux) counts as done, with one `warn` per destination volume (by `st_dev`) per operation. Network and FUSE mounts are the likely ones, and failing there would turn every move onto them into a failed move with duplicated files. Any other errno (`EIO`, `ENOSPC`, `EROFS`, `ENXIO`, `ENOENT`, `EACCES`, and so on), and any file it can't stat, open, or sync, is an `Err`. Pinned by `../durability_tests.rs`, a per-platform errno table included.

❌ Never let `already_synced` skip a directory fsync: a Mac → USB copy is always chunked, so every file is in the set, and a move would then delete its sources with no directory entry on disk. FAT/exFAT (the usual stick) keep no journal to replay a lost entry.

**Per-path specifics:**
- **Local copy** (`copy_files_with_progress_inner`): `copy_single_item` populates `already_synced` from `StrategyCopyOutcome::already_durable`, so the data sync reaches only the strategies that don't flush themselves (Linux `copy_file_range`, the std fallback); the directory fsyncs run for every strategy. A copy deletes nothing, so an `Err` only logs a `warn` and the copy completes. Pinned by `copy/copy_tests.rs::local_copy_fsyncs_its_created_directories_and_ignores_a_flush_failure`.
- **Cross-FS move** (`move_with_staging`): Phase 2 copies to staging (recording staging files and dirs), Phase 3 moves the staged items out. Before the flush, `landed_at` maps every recorded staged path to where it lives now. Phase 3 records `landings` (staged → landed) for each conflict resolution and each rename a merge made (`MoveTransaction::renamed_items`), and `discarded_staged` for what a Skip threw away. The nearest recorded ancestor decides: a landing maps the rest of the path under it, a discard drops the path (nothing of it landed), and with neither the name carried over from `staging_dir` to `destination`. `destination` gets its fsync as the parent of every top-level landing.

**Gotcha/Why exact landing paths**: a bare `staging_dir` → `destination` prefix swap maps a folder Renamed onto a same-named FILE to `destination/name/…`, under the user's file. That fsync answers `ENOTDIR` and fails a move that landed fine, while the real `name (1)` goes unflushed; a Skip onto a file does the same. Pinned by the rename, merge-rename, skip, and merge-skip cells in `move_op/move_durability_tests.rs`, each of which also checks the user's clashing file keeps its bytes and is never flushed. The journal's created-dir rows still take the prefix swap.

**Decision**: `flush_created_destinations` runs BEFORE Phase 4's `delete_sources_after_move`, and Phases 4 and 5 run only when it answers `Ok` AND the destination volume is still in the mount table.
**Why**: The source originals are the only other copy of the data through Phase 3. A power loss or pulled drive before the new directory entries are on disk can leave the moved files absent from their final paths, recoverable only as orphaned blocks or a stray `.cmdr-staging-*` entry; deleting the source in that window loses them. On `Err` the move keeps every source and everything that landed (the transaction is already committed), logs a `warn` naming the path and errno, emits `write-error`, and answers `WriteOperationError::MoveNotConfirmed { path, errno, volume_name }`, built from the `FlushFailure` rather than re-deriving the errno, so the user reads "Cmdr kept your originals where they were" and not a generic failure. Phase 5 is skipped too, so the empty `.cmdr-staging-<op>` shell stays behind.

**The destination-listed gate (the second half of the same decision).** An `Ok` flush is not proof the drive is still there: the kernel can accept writes moments before a cable leaves, and the fsyncs that would have failed never ran. So between the flush and the first delete, `move_with_staging` asks the mount table about the destination side's root (`transfer_sides::TransferSide::has_left`, the same non-blocking `getfsstat` snapshot discovery and eject read). Gone ⇒ keep every source, emit `DeviceDisconnected` naming that volume, return. An unreadable mount table is NOT evidence of leaving and lets the sweep run; a guess must never gate a delete either way. Pinned by `move_op/move_vanished_tests.rs` (the gate, and its twin proving a mounted destination still completes).

**The sweep asks the same question at every "already gone" reading** (`source_sweep.rs`): `remove_landed_file`'s `NotFound`, `remove_swept_dir`'s failed stat and its own `NotFound`, the per-source presence check, and the `source_removed` flag the outcome event carries. All four answer identically for a file this move carried and for a mount that left, and the second reading would report a user's untouched originals as deleted. On a drive that left, the sweep stops with `SweepStopped { error, counts }`, and the counts (`removed` / `left`, in top-level items) ride out on `WriteErrorEvent::progress_at_stop` so the copy can say which files are still theirs to find. This matches the cross-volume move (`volume/move.rs`, which finalizes before deleting the source). Happy-path cost: one `fsync` per distinct final directory, since Phase 2 already synced the data. Cancellation handling is unchanged: Phase 4 still owns the in-loop cancel check and the `write-cancelled` emit. Pinned by `move_op_tests.rs::cross_fs_local_move_flushes_final_dests_before_deleting_sources` (a custom sink snapshots that the source still exists at the instant the `Flushing` event fires) and `move_op/move_durability_tests.rs` (which directories, an `EIO` keeping every source, the unsupported class completing).
...
Only the cross-FS move acts on a failed flush; the local copy and the same-FS move log it under `target: "write_durability"` and complete, since nothing is deleted after their flush. Delete and trash don't flush at all; see `../delete/CLAUDE.md`.
...
- **Every skip site calls it.** The bulk-skip preludes (`transfer_driver/sync_driver.rs`, `async_driver.rs`,
  ...
  and the deep-merge decline (`volume/merge.rs`'s `MergeChildDecision::Skip`, via `SourceProgress::skip_leaf`). Miss one
  and it spikes the reported speed instead.

**`SourceProgress::skip_leaf` is THROTTLED, where `LeafProgress::complete` is not.** The completion milestone
...
**Decision**: the window's WIDTH formula is unchanged, but the window now reaches inside a directory source — one `merge_ctx.rs::FileWindow` per operation, shared by every merge walker and by the concurrent driver's top-level file tasks.
...
**Decision**: Volume-path progress is LEAF-granular, and the accounting is split by scope in `transfer_driver/progress.rs`: a `LeafProgressLedger` per operation, a `SourceProgress` per top-level source (the name its events carry, plus its throttle clock), and a `LeafProgress` per file, minted where the file starts and settled exactly once. `copy_single_path` takes the `SourceProgress` and every file below it mints its own handle. `bytes_total` / `files_total` are the preflight LEAF totals, so the emitted numbers climb across leaves AND across the many leaves one directory streams at once: reported bytes are `finished + sum(in-flight leaves)`, and an operation-wide `Arc<AtomicUsize>` counts completed leaves. The serial drivers reseed `finished` from their own `bytes_done` per iteration (they count skipped sources the leaves never see); the concurrent driver doesn't, because its sources overlap and the ledger IS its tally.
**Why**: The serial path originally captured FROZEN snapshots (`bytes_done_so_far`, `files_done_so_far`) per top-level source. For a directory source every inner file emitted against the same `(0, 0)` snapshot, so the Size bar reset to 0 at each inner file and the File bar sat at 0 for the whole folder (observed moving a 9-file folder, ~10.6 GB, USB → SMB NAS). The frozen snapshot also can't survive multiple directory sources. The byte axis predates this: the move site once shipped a no-op `Continue(())` callback because the old move code sent `bytes_total = 0`; once `volume/preflight.rs` populated `bytes_total`, the Size bar pinned at 0 through a multi-minute upload (SMB dest, 3.7 GB file, "Moving... 0 bytes" the whole time) — DON'T reintroduce a no-op progress callback. One shared high-water slot per source came next, and it broke the other way once a directory started streaming its leaves concurrently: the bar showed whichever leaf was furthest along, and the next leaf to finish wiped it, so a 664 MB folder whose largest member was 259 MB sawtoothed between ~300 MB and ~80 MB. Hence a share per leaf. A leaf's `complete` also credits its exact size whatever the callback did, which is why no "the volume never invoked on_progress" compensation is needed any more. Pinned by `a_small_leaf_finishing_does_not_wipe_a_big_leaf_still_in_flight`, `leaves_in_flight_sum_instead_of_reporting_the_largest`, `test_cross_volume_copy_directory_source_progress_is_leaf_granular`, `cross_volume_move_directory_source_progress_is_leaf_granular`, `cross_volume_move_emits_intra_file_progress`, `test_cross_volume_copy_serial_emits_intra_file_progress`, and `test_cross_volume_copy_concurrent_emits_intra_file_progress`.

**Decision**: The per-file milestone (the bypass-throttle emit that guarantees the axes cross `N/N` even when chunked emits ate the throttle window) lives at the unit-of-work layer, not the driver. Serial volume paths: `LeafProgress::complete` fires it per leaf. Concurrent path: the task-complete branch (`copy_volumes_with_progress::Some(Ok(...))`). Sync local-FS path: `copy_single_item` via `copy::record_file_done`. The async driver's `Transferred` arm emits a per-source milestone ONLY when `DriverConfig::emit_per_source_milestone` is `true` — set for the same-volume rename-merge, whose `transfer_one` does a bare `rename` with no streaming and thus no closure emit, so the driver milestone is its only Copying event. The streaming paths set it `false`: a top-level-granular driver milestone after a directory source would regress the File bar (9/9 → 1/9). `drive_transfer_serial_sync`'s `Transferred` arm never emits (the closure owns it). Mirrors the sync/async conflict-resolution split (sync driver delegates to closure, async driver dispatches itself).
