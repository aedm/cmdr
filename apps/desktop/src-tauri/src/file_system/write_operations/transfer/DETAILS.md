...
2. **End-of-op pass.** Before `write-complete`, every handler that wrote bytes calls `durability::flush_created_destinations` (the same-FS move, which wrote none, takes the directory-only sibling below). It emits a `Flushing`-phase `write-progress` event (the FE renders **"Writing the last piece..."** instead of a bar frozen at 100%; see the FE doc), then:
   ...
   - Answers `Result<(), FlushFailure { path, errno }>`, trying every path before answering the first failure. A directory fsync failing with `ENOTSUP` or `EINVAL` counts as done with a `warn`: some filesystems refuse directory fsync, and blocking there would stop every move onto them from ever deleting its sources. Any other directory errno (macOS's distinct `EOPNOTSUPP` and `ENOSYS` included), and any file it can't stat, open, or sync, is an `Err`. Pinned by `../durability_tests.rs`.
...
**Gotcha/Why exact landing paths**: a bare `staging_dir` → `destination` prefix swap maps a folder Renamed onto a same-named FILE to `destination/name/…`, under the user's file. That fsync answers `ENOTDIR` and fails a move that landed fine, while the real `name (1)` goes unflushed; a Skip onto a file does the same. Pinned by the rename, merge-rename, and skip cells in `move_op/move_durability_tests.rs`. The journal's created-dir rows still take the prefix swap.
