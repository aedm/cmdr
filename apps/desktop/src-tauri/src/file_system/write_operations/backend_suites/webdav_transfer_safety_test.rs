...
/// The landing is `MOVE` with `Overwrite: F`, so the SERVER refuses the name
/// another writer took while our bytes were on the wire.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "needs the WebDAV fixture stack: apps/desktop/test/webdav-servers/start.sh (webdav-fixture)"]
async fn webdav_integration_a_name_taken_mid_upload_is_never_replaced_under_skip() {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "needs the WebDAV fixture stack: apps/desktop/test/webdav-servers/start.sh (webdav-fixture)"]
async fn webdav_integration_a_name_taken_mid_upload_is_never_replaced_under_rename() {
    let (remote, dir) = fixture().await;
    a_name_taken_mid_upload_is_never_replaced(remote, dir, ConflictResolution::Rename, STAGED_ON_EVERY_BACKEND).await;
}
