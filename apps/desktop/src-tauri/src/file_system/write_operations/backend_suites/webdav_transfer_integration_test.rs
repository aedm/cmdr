//! Real copies between a local disk and a live WebDAV server, driven through
//! the app's own `copy_between_volumes`.
//!
...
//! with the SFTP suite so a claim can't hold on one backend and quietly rot on
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "needs the WebDAV fixture stack: apps/desktop/test/webdav-servers/start.sh (webdav-fixture)"]
async fn webdav_integration_copying_a_directory_tree_onto_a_server_lands_it_intact() {
    let (remote, dir) = fixture().await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "needs the WebDAV fixture stack: apps/desktop/test/webdav-servers/start.sh (webdav-fixture)"]
async fn webdav_integration_copying_a_directory_tree_off_a_server_lands_it_intact() {
    let (remote, dir) = fixture().await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "needs the WebDAV fixture stack: apps/desktop/test/webdav-servers/start.sh (webdav-fixture)"]
async fn webdav_integration_a_cancelled_upload_leaves_nothing_behind() {
    let (remote, dir) = fixture().await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "needs the WebDAV fixture stack: apps/desktop/test/webdav-servers/start.sh (webdav-fixture)"]
async fn webdav_integration_an_overwrite_answer_replaces_the_destination_bytes() {
    let (remote, dir) = fixture().await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "needs the WebDAV fixture stack: apps/desktop/test/webdav-servers/start.sh (webdav-fixture)"]
async fn webdav_integration_a_pre_existing_destination_still_probes_each_name() {
    let (remote, dir) = fixture().await;
