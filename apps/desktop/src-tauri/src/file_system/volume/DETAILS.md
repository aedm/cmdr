...
- `begin_scan_session()` / `end_scan_session()`: default-no-op async hooks the indexing lifecycle
  ...
  extra TCP sessions the cold walk lists across (canonical: `crates/cmdr-smb/DETAILS.md` § "SMB scan-connection pool"); the pool
...
- `connection_liveness()`: has this volume's connection been PROVEN dead, as opposed to merely slow to answer? Three-valued (`None` = no evidence either way, which is what every backend but SMB answers). It exists to gate the one aggressive thing the transfer watchdog does — ending the wait on a task that has stopped moving — and a wrong `Dead` kills healthy slow transfers, so the bar is proof rather than suspicion. ❌ Never answer it from elapsed silence: a large write to a loaded spinning-disk NAS is legitimately slow and looks identical on the wire to a dead server. Telling the two apart needs a keepalive (an ECHO answered inside a window) and a clock that counts every inbound byte as it lands; `smb2` 0.23.0+ exposes exactly that as `Connection::liveness()`, and `SmbVolume` maps it (`crates/cmdr-smb/src/volume/liveness.rs`). A missed probe alone is still never death: a busy NAS drops probes. The mapping, and everything the answer gates: `write_operations/transfer/DETAILS.md` § "The watchdog ACTS".
- `connection_bytes_received()`: payload bytes the connection has received, counted as they land (a response still arriving included). `None` for every backend but SMB. A RATE source for the transfer watchdog while nothing lands, ❌ never progress: those bytes are unverified until their frame completes, and the count is connection-wide. It can drop (a rebuilt session counts from zero), so a reader only compares it with its own earlier readings. What the watchdog does with it: `write_operations/transfer/DETAILS.md` § "The stall signal".
- `rerooted(new_root)`: build an equivalent volume rooted somewhere else, or `None` (the default) for "leave me where
  ...
  session (`crates/cmdr-smb/DETAILS.md` § "Re-rooting a share"). Declining is not a failure mode: a backend whose transport is
  anchored to the old root keeps its registration instead of being handed a root it can't serve.
- `space_poll_interval()`: recommended interval for the live disk-space poller (`space_poller/`). Default 2 s (local volumes). `SmbVolume` and `MtpVolume` override to 5 s. `InMemoryVolume` returns `None` (no polling). The poller uses this to tick each volume at its own cadence.
...
- [ ] Implement `get_space_info()`: for the volume usage bar and pre-copy space checks. Answer `SpaceInfo::Bounded` where the backend knows a capacity, `SpaceInfo::Unbounded { used_bytes }` where the storage has no ceiling but reports what it holds (a quota-less WebDAV account), and `VolumeError::NotSupported` where the protocol can't say at all. ❌ Never zeros: the pre-flight reads a zero `available` as "no room" and refuses every copy. A backend spanning several filesystems (ADB: a read-only system image at `/` beside the shared storage) also overrides `get_space_info_at(path)`, which the pre-flight asks about the destination folder. A volume over a parent (`ArchiveVolume`, `GitPortalVolume`) forwards `get_space_info_at` to the parent with its own path there. Answer `write_access_at(path)` too if your protocol can tell, without writing, that a folder takes no writes; otherwise leave the `Unknown` default, ❌ never a guess.
...
If the SDK's download handle holds a borrow against the session (like `smb2::FileDownload<'a>` borrowing `&'a mut Connection`), you can't stuff it into a `'static` struct. Use a background producer task that holds an `OwnedMutexGuard` over the session, drives the download, and feeds chunks through a bounded mpsc channel. The consumer half is shared: `cmdr_fs::volume::ChannelReadStream` (the channel, the drop-cancellation, the progress counters), so a backend writes only its producer. **Examples**: `crates/cmdr-smb/src/volume/streams.rs` → `open_smb_download_stream`, and `crates/cmdr-sftp/src/volume/streams.rs` → `open_read_stream_impl`.
...
**Unmounted but not powered down is a silent `Ok`.** When every mount of the disk left the table but the hardware
stayed attached (API-driven ejects of APFS media can do that: `NSWorkspace` ejects the synthesized container and leaves
...
**Decision**: `report_identity_conflict` logs at WARN, not error
**Why**: error level is the auto-report threshold (`error_reporter/CLAUDE.md`: a recoverable, expected, non-user-impacting failure belongs at warn). The common case here is a share macOS mounted twice, which is all three, and the message itself calls it expected. At error level one machine's remount loop uploaded 98 bundles in a week, which drowned the channel the alert exists to reach. A genuine clone would deserve louder, but nothing at this layer can tell a clone from a double mount, so the line stays loud in the log and names both.
...
**Why**: `NotFound`, `PermissionDenied`, and `AlreadyExists` are defined to carry the PATH, and `transfer_error.rs::map_volume_error` forwards that payload into `SourceNotFound { path }`, which the frontend renders as the name of the file the user is missing. A bare `io::Error` has no path inside it, so a blanket `From` could only put the errno's own sentence there, and every `?` on an io error silently did. Removing the impl makes the compiler ask "which path?" at each site; `from_io_without_path` is the explicit escape for a pipe, socket, or already-open handle, where the three path-carrying variants have nothing honest to carry. Held by `cmdr_fs::volume::conformance::assert_not_found_carries_the_path`, now wired into every backend. `from_io_at` also lifts the kinds with a typed home (read-only filesystem, full disk or quota, a name the filesystem can't hold, is-a-directory), the way `cmdr-adb` does, so a copy onto a mounted installer image reads as a read-only destination instead of a generic failure. Every other errno stays on `IoError`, because `note_root_failure` and transfer retry dispatch on it; the list and that guardrail live on `from_io_at`'s doc comment.
