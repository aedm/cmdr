...
- Eject (1): `eject` (`volumeId`; gate `Open`). A thin adapter over `file_system::volume::eject::eject` — parity with the one-click Eject button. The backend refuses honestly (surfaced as errors, not false OKs) while a write op reads from or writes to the volume (`EjectError::Busy`) and for non-ejectable volumes. A call for a volume whose eject is still running (from the UI or another call) joins it and answers with that eject's result, never a second teardown; `Open` because it's a reversible, one-click runtime action touching no persistent state. A refused unmount is the one answer an agent can act on, so it sets `data` to `{ "outcome": "unmountRefused", "holders": <HolderScan> }` — the same typed value the frontend gets, tag and all. ❌ Never branch on the message, and ❌ never read `holders` with an empty `named` as "nobody is holding it": only the `complete` arm means that, while `incomplete` means the scan couldn't cover every mount (`file_system/volume/DETAILS.md` § "A refusal names who held the drive").
