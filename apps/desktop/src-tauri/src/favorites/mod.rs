...
//!
//! Owns the ordered `favorites.json` store that backs the frontend's favorites menu (⌃D).
...
//! commands in `commands/favorites.rs`, which re-emit `volumes-changed` so both panes' menus
//! update live.
//!
