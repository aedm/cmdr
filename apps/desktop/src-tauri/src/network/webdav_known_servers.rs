...
impl KnownWebdavServer {
    /// What the UI calls this server: its name, or `username@host` with the host
    /// read off the URL (`saved_server_fields::server_label`).
    pub fn label(&self) -> String {
        let host = self
            .endpoint()
            .map_or_else(|| self.url.trim().to_string(), |(host, _)| host);
        saved_server_fields::server_label(&self.display_name, &self.username, &host)
    }

    /// The `(host, port)` this entry's URL names, the port being the scheme's own
    /// when the URL gives none. `None` when it isn't an `http`/`https` URL.
    ///
    /// ❗ Through the crate's own params, so the pair is exactly the one a dial
    /// derives the volume id and the app-root prefix from.
    pub fn endpoint(&self) -> Option<(String, u16)> {
        let parsed = url::Url::parse(self.url.trim()).ok()?;
    ...
    /// Whether its name only repeats its own address: its URL (any path, with or
    /// without the account in it), a `dav`/`davs`/`webdav` spelling of it, or
    /// `username@host[:port]`.
...
pub fn load_known_webdav_servers<R: tauri::Runtime>(app: &tauri::AppHandle<R>) {
    let Some((mut store, path)) = server_list_file::load(app, "webdav_known_servers.json") else {
        return;
    };
    ...
    *known().lock_ignore_poison() = store;
    ...
    if cleared > 0 {
        log::info!(target: "volume", "unnamed {cleared} saved WebDAV server(s) whose name only repeated their address");
        save();
    }
}
...
fn clear_address_shaped_names(store: &mut KnownWebdavServersStore) -> usize {
    let mut cleared = 0;
    for entry in &mut store.known_webdav_servers {
