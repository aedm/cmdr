...
`CMDR_SECRET_STORE=file` forces the plain file backend in dev and E2E builds (set by `tauri-wrapper.ts`); release builds ignore it (`secrets/CLAUDE.md`).
...
**The open panes on the share are re-read through the new backend.** After `register` (unless the registry kept the incumbent), `respell_listings_on_volume` re-lists every cached listing on that volume id. A listing the kernel mount produced carries its spelling in its own path and every entry path (the kernel decomposes every name), which the direct connection, matching names byte-for-byte, misses: without the re-read, every accented entry in a pane open during the upgrade opened nowhere. How the re-read re-keys the listing and tells the pane: `file_system/listing/DETAILS.md` § "A pane path the volume stores another way". Pinned end to end by the Docker cell `smb_upgrade_respell_test.rs`.
...
`known_shares` answers "what username should the sign-in sheet pre-fill" with
...
`server_name.to_lowercase()` while the frontend looked up `host.name.toLowerCase()`, and neither is
...
(`set_auto_reconnect`, shaped like `set_pinned`) so a menu can't write back a stale record. WebDAV mirrors both. The
`*_integration_the_reconnect_switch_moves_the_saved_entry_and_the_live_volume` cell in each wiring's test file holds
both copies moving, both ways, through `set_place_auto_reconnect`. §
...
- **`mdns-sd` is a patched fork, not the crates.io release**: `vendor/mdns-sd` is swapped in via `[patch.crates-io]` to stop an endless multicast-join retry loop on VM bridge interfaces. Read `docs/notes/mdns-sd-multicast-join-retry-loop.md` before upgrading the crate or touching the patch.
