...
/// What every [`DeviceVolumeProvider`] method returns: a boxed future borrowing
/// the provider (and its arguments) for `'a`. Boxed rather than `async fn` so
...
///
/// Every method answers from the provider's own cached state. The registry
/// holds `Arc<dyn DeviceVolumeProvider>`.
...
pub(crate) fn notify_devices_changed(provider_id: &str) {
    // allowed-pluralize-noun: `{provider_id}` is a provider name, never a count.
