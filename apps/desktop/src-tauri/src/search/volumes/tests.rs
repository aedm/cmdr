//! Per-volume registry tests: loading a non-root volume straight from its persisted
//! DB (mount root from meta), the missing-DB honesty signal, and per-volume
//! importance weight loading.
