...
`fixtures.ts` wraps each test in an auto fixture that updates the main window's OS title via the standard Tauri window
plugin (`plugin:window|set_title`). While a test runs the title becomes `<base> (Running: <name>)`; after it returns,
`(FINISHED)` is appended. The base title is captured once per worker on the first decoration so suffixes don't
accumulate. The required permissions (`core:window:allow-title`, `core:window:allow-set-title`) are added to the
...
## Video recording is off unless the run asks for it

Upstream's `tauriPage` fixture starts a 15 fps native frame capture for EVERY test and, at the end, runs an ffmpeg
encode, reads the mp4 off disk, and attaches it to the report — on passing tests too. Three shards run at once, so that
is three continuous frame captures plus a per-test encode on a machine somebody is using, for videos nobody watches.

So `cmdrTestGuards` (`fixtures.ts`) stops it at the start of every test, and keeps it exactly where it earns the cost:

- **a retry attempt** (`testInfo.retry > 0`), where the video is the diagnosis;
- **`CMDR_E2E_KEEP_RECORDING=1`**, which the check runner's isolation re-run sets.

Three things the shape depends on:

- **The stop can only live in a fixture that DEPENDS on `tauriPage`.** The recorder starts inside `tauriPage`'s own
  setup, so `cmdrDeadAppGate` — which destructures nothing precisely so it runs before that (§ the file header) — has
  nothing to stop yet.
- **It is best-effort and never fails a test.** Upstream's own teardown calls `stopRecording()` again; the second call
  simply finds nothing recording and is swallowed there.
- **The native failure screenshot is untouched.** Upstream takes it from the live window after a failing test, with no
  dependence on the recording, so every failure still carries its picture.

The two capture drivers (`i18n-capture.spec.ts`, `marketing-shots.spec.ts`) call `stopVideoRecording` themselves rather
than relying on the default: a camera run is the one place a recorder actively harms the output (it competes for
CoreGraphics work with the very shots it would degrade), and `marketing-shots.spec.ts` runs on `captureTest`, which has
no auto fixture at all.
...
## Saturation under concurrent builds, and the two stages that tell it apart

The E2E lanes share a machine with whatever else is compiling, and the suite's per-test waits (5 s for a dialog, 3 s for
a toast) are wall-clock. Under load the failures are numerous, scattered across unrelated specs, and different every
run: that pattern is saturation, not a regression. Measured on one branch: 25 failures with `desktop-e2e-playwright`,
`desktop-e2e-linux`, and `rust-tests-linux` running together, 1 failure running them one at a time, and the MTP shard
went 34/34 green.

**The lane now delivers that verdict itself, in two stages, so nobody has to re-derive it.**

1. **The retry** (`retries = 1` in `playwright.config.ts`, both lanes). A rescued pass is reported `flaky`, never a
   pass, and downgrades the run to a **warn** naming every rescued spec (`scripts/check/checks/e2e-flaky.go`). It fires
   about a second after the failure, inside the same shard, with all three Tauri instances and all three recorders still
   competing, so it catches a one-second hiccup and nothing longer.
2. **The isolation re-run** (`scripts/check/checks/e2e-contention.go`), on a red run only. Once every shard has
   finished, each failing spec FILE re-runs alone against its own shard's still-live app: first at the unchanged 15 s
   timeout, then (only if it still fails) at 4x. Passing alone means the lane was starving it, which downgrades the run
   to a **warn**; failing alone with 4x keeps it red. The output names which specs are which, prints each one's record
   from the per-test log, and gives the load average at re-run time.

So a red `--include-slow` already carries its own verdict: read the headline before re-running anything. What still
needs a human is a `too-slow` or `real` verdict, and the control run against the parent commit that decides whether your
change caused it. Full ladder and its gotchas: `scripts/check/checks/DETAILS.md` § "The Playwright isolation re-run".

**The lane also runs niced.** Both the shard apps and the Playwright processes launch at `nice -n 5`, which yields CPU
only when it's contended, so the suite gets out of the way of whoever is using the machine. That trades some suite
wall-clock for responsiveness, and the isolation re-run is what makes the trade safe: a spec starved by it is re-run
...
find it through `selectContentExpr` (the trigger's `aria-controls`), never a selector nested under the toolbar or
dialog. `selectRowHits` hit-tests the centre of every row with `elementFromPoint`, which is what a real click lands on.
Call `runAnimationFramesOnTimers` BEFORE opening the menu: zag places it, and writes its stacking styles, from a
...
against a victim instead of its culprit. They live on an auto fixture (`cmdrTestGuards`), which applies to every test
importing this `test` and tears down after the spec's own `afterEach` hooks.
...
`open_in_editor` (F4's launch into whichever editor is chosen, used by the `file.edit` command and the new-file flow's
"open the freshly created file" step), compile to variants that push the path into an in-process store instead of
shelling out to `open` / `xdg-open`. Real launches would leak TextEdit/Preview windows the suite can't close, unbounded
across runs (this is why the new-file round-trip test once left a TextEdit window per run). A spec asserts open intent
via the `getOpenedPaths` / `clearOpenedPaths` helpers (backed by the `e2e_opened_paths` / `e2e_clear_opened_paths`
commands, both feature-gated). Both commands funnel into one `open_mock` store, so `getOpenedPaths` sees every external
open regardless of which command made it. A new user-visible "open externally" flow MUST route through a backend command
gated the same way, never the frontend opener plugin (which really launches), or it re-introduces the leak. See
...
Two helpers, one per side of the IPC boundary, and the name of each says which side it proves
(`helpers/app-lifecycle.ts`):

- **`waitForBackendOperationsToSettle(tauriPage)`** is `drainOperations`'s non-destructive sibling: it polls
  `list_operations` until it is empty and cancels nothing. It proves the OPERATION is over, and nothing else.
- **`waitForTransferUiToSettle(tauriPage)`** waits for that AND for every transfer surface (the setup dialog, the
  progress dialog, the queue window's conflict prompt) to leave the screen. It is what a frontend assertion needs: a
  dialog being gone, the follow-up rename editor, or the next gesture reaching the app rather than a modal. It names the
  surfaces rather than probing `.modal-overlay`, so a spec that legitimately ends in the rename editor is settled rather
  than leaking. It takes the backend half first on purpose: polling for the dialogs alone would pass vacuously against a
  gesture whose dialog hasn't opened yet.

**Which one is a real choice, because there is NO happens-before edge between them.** The backend's registry emptying
says nothing about the webview having taken `write-complete` off the event bridge, closed the dialog, and raised the
toast: that lag is ~0 ms on an idle box and was measured at 650 ms in a loaded lane, where an instant read caught the
dialog on its last frame. A spec that waits on the backend and then asserts on the DOM therefore passes on a fast
machine and fails under load, reading as flake rather than as the missing wait it is. Hence the rule in `CLAUDE.md`: a
wait helper names the boundary it waits on, and the assertion after it picks which one to call.

**The trap they close is that a write op looks finished long before it is.** Two things a spec naturally waits for both
land mid-operation:
