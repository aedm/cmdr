...
- **Dialog gallery listener.** `debug-open-gallery-dialog` (same block, which is gated
  `import.meta.env.DEV || __CMDR_E2E_BUILD__` so every E2E build carries it too) opens a soft dialog over the main
  window with fixture data, for design review from Debug > Soft dialogs. The handler seeds the gallery store and focuses
  the main window from this side (the Debug window's capability set is minimal and permission failures are silent). The
