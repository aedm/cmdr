...
Its lists come from `third-party-packages.gen.json`, generated by the `third-party-notices` check alongside the
repo-root `THIRD-PARTY-NOTICES.md`: `vendored` from the hand-kept `scripts/check/checks/third-party-vendored.json` (the
icon sets, fonts, and code snippets Cmdr ships as files, which no lockfile knows about), `rust` from `Cargo.lock`, and
`npm` from `pnpm-lock.yaml`. **Never hand-edit the JSON**: the check regenerates it and fails CI on drift, so a new
credit goes into the vendored list (`scripts/check/checks/DETAILS.md` § "Vendored credits"). Full license texts, and a
...
- **One grid spans every list.** `.packages` is `grid-template-columns: minmax(0, 1fr) auto auto` (name takes the slack,
...
The three lists render from one `{#each}` over a `sections` array, vendored credits first (a few rows, so they show on
open, while the two jump buttons cover the long lists below), rather than a snippet called per list: `{@render}` of a
