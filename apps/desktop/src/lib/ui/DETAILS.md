...
progress, transfer error, operation log, go to path, new file/folder, alert, and the query dialogs). It puts a grab band
on each exposed edge and, when both axes are free, on each corner — `'horizontal'` simply renders fewer bands, which is
also what keeps the height content-driven. A dialog never remembers the size it was dragged to; it reopens at its
`containerStyle` size. Four things make the drag work:

- **Each band straddles the panel edge** (4px out, 3px in), the way a macOS window's resize zone does, so the pointer
  finds it without hunting and a scrollbar at the body's right edge stays grabbable. The overhang is why the panel
  itself no longer clips: `overflow: hidden` moved down to `.modal-content`, which is also where the rounded-corner clip
  for `resizable` and `fillBody` now lives. ❌ Don't put `overflow: hidden` back on `.modal-dialog` — it cuts every band
  in half.
...
- **The floors are the panel's `min-width` (360px, raised in `containerStyle` when content needs more, as the progress
  readout's fixed columns do) and a measured height floor**: the title bar plus footer plus 60px of body. The panel
  carries no CSS `min-height`, so every dialog still opens at its natural height instead of padding out to a number.

The bands carry their direction in `data-direction`, not in a class per edge, so `.resize-band` stays statically
matchable for Svelte's CSS scoping and the `css-unused` check. Their `cursor` is the single source for which arrow a
direction shows: the drag reads it back off the band and puts it on `<body>` for the duration.
...
- `fillBody` makes the panel a flex column whose `.modal-content` clips (so full-bleed bands clip to the 27px radius)
  and the body a flex column that takes the slack; the caller caps the height via `containerStyle`. `resizable` covers
  the same ground for a body that scrolls as a whole; combined, `fillBody` wins the body and `resizable` contributes
  only the bands and the size floors, so the inner region does the scrolling and there's never a scrollbar inside a
  scrollbar.
...
- `{ kind: 'toggle', checked }`: an on/off setting (the row-menu toggles in `file-explorer/navigation/row-menu.ts`). The
  row is a `menuitemcheckbox` with `aria-checked`, ON OR OFF, so an off toggle says "unchecked" rather than reading as
  an action. `checked` lives inside the kind, so an off toggle and a plain row can't share a shape.
- `{ kind: 'current' }`: the row you're on (the switcher's volume). The row stays a `menuitem`, drawn checked, with
  `aria-current="location"`.
- No `check`: a plain `menuitem`, no `aria-checked`.
...
- **One level of submenu**, and no type-ahead, virtualization, or radio items: nothing needs them, and each is addable
  without moving the seams (a radio would be a third `MenuItemCheck` kind, for rows hosting no controls).
...
contract of a primitive at a glance. It's reachable in the running app via Debug window (`⌘⇧D`, dev builds) →
"Components", or directly in a browser tab at `http://localhost:<port>/dev/components`.
