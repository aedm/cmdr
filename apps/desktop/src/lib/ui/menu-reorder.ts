/**
 * Pure reorder math for a `Menu` section marked `reorderable` (`lib/ui/Menu.svelte`).
 * Kept free of the DOM so the index arithmetic is unit-testable; the controller
 * (`menu.svelte.ts`) feeds it measured row midpoints and hands the result to the
 * caller's `onReorder`, which is what persists.
 */
