...
- **F4 (edit) goes through `editor-open.ts`**: `openInEditorOrExplain(volumeId, rowPath)` hands a file to
  `$lib/text-editor`'s `openFileInEditor` only when its path is a plain filesystem path AND `rowIsOsVisible` holds, and
  otherwise shows `fileExplorer.edit.notOnThisMac` (a phone, a server, an archive's insides, the `.git` portal),
  pointing at F3. The guard lives here because it's a pane question; WHICH app opens the file, the missing-app fallback,
  and every toast around the launch belong to `apps/desktop/src/lib/text-editor/DETAILS.md`, and the setting never
  widens what the guard accepts. The pane's `file.edit`, the search-results F4 (against the snapshot's `volumeId`), and
  the new-file auto-open all call it; its `EditorPressOutcome` is `opened` (an app was asked, a fallback included),
  `refusedNotOnThisMac`, or `launchFailed`, and `file.edit` counts `editor_opened` only on `opened`. ❌ Never call
  `openInEditor` on a row directly: an editor launch on a scheme path does nothing and says nothing.
...
- **Clipboard** (`clipboard-operations.ts`): the snapshot-clip path gate off `kind === 'search-results'`; the routed
  ...
  **The snapshot-clip branch gates TWICE, in this order** (`snapshotClipboardIsRefused`). First the PATH SCHEME:
  ...
  path alone. Then the VOLUME the snapshot's search covered (`SearchSnapshot.volumeId` →
  `isSchemePathClipboardRefusal`), because the pane's own volume id is the virtual `search-results` and a search covers
  any volume with a persisted index, MTP storages and ADB phones included. Either gate refuses the WHOLE set if any row
  offends; a partial copy under a toast claiming success is worse than a refusal.
...
1. ❗ **The volume store's row moves first** (`applyVolumeRootChanged`). The event arrives before the debounced
   ...
   would be refused against the old one.
...
`NetworkMountView` mirrors a mount that didn't go through the same way, as `PaneState.mountError`
(`{ share, reason, message }`: the typed `MountError` tag beside the sentence the pane shows, which is in the UI's
language) — the "Couldn't mount share" pane and the login form an auth-class failure routes to alike. Without it a
failed mount is invisible from `cmdr://state`: the pane's `path` and `files` still describe the share list either view
replaced, so a reader sees a pane that simply didn't move. The clear is an explicit push of its own rather than
something the next view is trusted to do: `PlacesBrowser` only pushes once it has a share list, so a host that went
quiet between the failure and Back pushes nothing at all, and a `mountError` outliving its pane misleads a reader worse
than the silence the mirror replaced. `NetworkMountView.test.ts` holds that line.
