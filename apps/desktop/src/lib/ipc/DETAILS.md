...
only `undefined` as "absent" (for example `gitMeta !== undefined`, `?? entry.size` chains, or a `String(value)` render
path) accepts `null` as a real value and surfaces literal `"null"` in the UI. The Group A migration (commit `dc5f0b47`)
flipped the shape across the board; size and date columns rendered the literal text `"null"` until consumers were
switched to `!= null` checks.
