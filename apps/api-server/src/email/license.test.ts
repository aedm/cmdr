...
describe('getLicenseDescription', () => {
  it('promises no end date for a perpetual license', () => {
    const line = getLicenseDescription('commercial_perpetual', 'MBition')

    expect(line).toBe('Your perpetual commercial license for MBition is valid forever, with no renewal and no expiry.')
  })

  it('names the end date for a dated license, and says nothing renews it', () => {
    const line = getLicenseDescription('commercial_subscription', 'MBition', '2026-12-15T18:05:57.922Z')

    expect(line).toBe("Your commercial license for MBition is valid until 2026-12-15, and doesn't renew automatically.")
  })
