...
- [ ] 2026-09-17 `detach-any-volume.md` - **An agent can open a remote session and has no way to close one, because the
      ...
      (`device_volumes.rs`), ❌ not the `Volume` trait, which lives in a crate that carries no `tauri` while the SFTP and
      WebDAV teardowns are app-side wiring. M1 types every refusal (30 min, useful alone) → M2 one detach registry and a
      two-case decision → M3 SFTP and WebDAV register, and MCP inherits it → M4 a fake-provider test proving the next
      backend (S3) needs no edit in `eject/`. The disk flight, the busy gate, the in-flight join, and `run_teardown`
      stay exactly where they are. About half an agent-day.
