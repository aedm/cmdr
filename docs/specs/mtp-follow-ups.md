...
- **Solution**: David, with an Android phone over USB, in order: (1) connect and list a folder; (2) copy a file to the
  phone and one back off it; (3) delete a folder that still has children (it must refuse and delete nothing); (4) unplug
  mid-copy; (5) lock the phone's screen to trigger a session reset (the volume must stay in the switcher and come back
  on its own); (6) replug into a DIFFERENT USB port (the index must re-match by serial, not rescan); (7) toggle the MTP
  setting off and on. Throughout, watch the log for registration-before-event-loop order. File a bug per failure.
