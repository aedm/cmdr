...
- **`EnsureGoTool(rootDir, name, installPath)`** — returns the path to the tool built from the pinned version,
  installing it into `node_modules/.cache/cmdr-go-tools` when what's there doesn't match. Used for staticcheck, nilaway,
  etc. `installPath` MUST pin a specific version (`@vX.Y.Z` or a pseudo-version); an unpinned path or `@latest` is a
  hard error, not a fallback. Same rule applies to `cargo install` calls inside checks: pin both `--version` and
  `--locked`. It ignores a same-named binary on `PATH` and installs repo-locally, so a bumped pin takes effect on the
  next run with nothing to do by hand, and the user's own `~/go/bin` is left alone. Currency is decided by
  `go version -m` on the binary: the module version must equal the pin AND the recorded Go toolchain must equal
  `MiseGoVersion`, since a tool built by an older Go can't parse a newer stdlib (that mismatch reads as
  `method must have no type parameters` inside `math/rand`, which names neither the tool nor its age).
...
```json
{
  ...
  "files": {
    "apps/desktop/src/lib/foo/bar.ts": { "lines": 412, "reason": "one state machine whose steps share locals" }
  }
}
```
...
- `files` entries suppress the failure up to the recorded line count plus a 10% growth buffer; beyond that the file is
...
yellow phase before red rather than going straight to red at their (already generous) warn line. `isTestFile` classifies
a path by its own naming convention, never by content, so an inline `#[cfg(test)] mod tests` block inside an ordinary
`.rs` file does NOT count as a test file:
...
**A long file that isn't allowlisted FAILS the check** (as does an entry with no reason); the shrink-wrap path stays
advisory, a local auto-fix and a CI warn, since that's bookkeeping rather than somebody's unfinished work. A warn had no
owner: it outlived the run that caused it and reached David as a separate triage pass. An agent may add an entry itself
now, with a reason, and says so in the commit; see `.claude/rules/file-length-allowlist.md` (repo-level) for the rule.

## CLAUDE.md length

`claude-md-length` (`IsFast`) keeps the push tier lean: it fails when any `CLAUDE.md` exceeds 600 words. Each
...
`claude-md-length-allowlist.json` has one `files` section mapping path → `{"words": N, "reason": "…"}`, with the exact
same shrink-wrap, mandatory-reason, and `TODO: ` discipline as file-length (`claudeMdWordLimit` mirrors
`fileLengthLimit`): a file is suppressed up to its recorded count plus a 10% buffer; local runs drop
dead/under-threshold entries and ratchet >10%-slack entries down, carrying the reason through; CI reports what a local
run would change. The fix for an oversized `CLAUDE.md` is to move depth into `DETAILS.md` rather than bump the number,
and the reason on any entry that does get added should say which of the two it is.
...
Two lanes over one shared core (`jscpd.go`): `jscpd-rust` (`desktop-rust-jscpd.go`) and `jscpd-frontend`
...
{
  "exempt": {
    "apps/desktop/src-tauri/src/intl/shipped_locales.gen.rs": "generated by …; the clone tracks the shipped-locale table"
  },
  "pairs": { "a.rs ↔ b.rs": 14 }
}
```

**Decision**: a generated file's duplication gets a reason, never a count. **Why**: a count is a budget somebody is
meant to defend by extracting the shared code, and nobody can extract anything from a file a script rewrites. It would
also need re-approving on every input growth — the `zh`/`zh-Hant` clone in `shipped_locales.gen.rs` grows with the
locale table, so a number there would warn again the next time a catalog lands, teaching people to bump it by reflex.

- An exempt pair never warns, at any size, and `findJscpdRegressions` skips it before it looks at `pairs`.
- Shrink-wrap KEEPS an exempt entry that currently has no duplication (a regeneration that happens to emit no clone
  doesn't make the file hand-written), and drops one only when a path behind the key is GONE — a dead exemption silently
  suppresses whatever moves in under the old name.
- A key in both sections loses its `pairs` entry on the next local run; `exempt` wins, and a stale number beside it
  reads as a live budget.
- Adding an entry needs David's OK, same as raising a `pairs` number.

The exemption is for a file that is BOTH generated and tracked. A gitignored build artifact never reaches either lane's
roots, so it needs no entry.
...
`write_operations/backend_suites/mtp_archive_test`, `mtp_scan_oracle_tests`, the transfer cells) and `cmdr-mtp`'s own,
which the feature reaches through `cmdr-mtp/virtual-device`. Without it `cargo nextest` silently filters them out, so
they protected nothing while looking like coverage. It's test-only (never enters a production build) and costs ~2-4 s on
a ~27 s suite, so it's the cheapest set that keeps the test lane honest. It MUST stay package-qualified
...
- **Tests' temp folders stay on the container's overlay disk, where one fsync costs ~2.4 ms** (OrbStack, measured
  ...
  tmpfs to dodge it: a temp folder on its own mount resolves to its own volume, which fails every test asserting a temp
  path sits on the boot volume (`commands::volumes::tests::resolve_location_*` did, the one run it was tried). A test
  that syncs hundreds of times gets fixed at its source (one transaction, an in-memory index, a seeded file).
