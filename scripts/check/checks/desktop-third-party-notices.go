...
// lockfiles plus the hand-kept vendored credits (`third-party-vendored.go`),
// and fails in CI if the committed copy is stale.
//
...
func RunThirdPartyNotices(ctx *CheckContext) (CheckResult, error) {
	...
	// First, since it's only a file read: a broken credit fails before cargo-about runs.
	vendored, err := loadVendoredCredits(ctx.RootDir)
	if err != nil {
	...
	generated := renderNotices(rust, npm, vendored)
	generatedPackages, err := renderPackagesJSON(rust.packages, npm, vendored)
	if err != nil {
		return CheckResult{}, err
	}
	...
	summary := fmt.Sprintf("%d Rust crates, %d npm packages, %d vendored credits, %d license texts",
		len(rust.packages), len(npm), len(vendored), len(rust.texts))
...
func renderPackagesJSON(rust []attributedPackage, npm []attributedPackage, vendored []vendoredCredit) ([]byte, error) {
	payload := struct {
		Comment  string              `json:"_comment"`
		Vendored []attributedPackage `json:"vendored"`
		Rust     []attributedPackage `json:"rust"`
		Npm      []attributedPackage `json:"npm"`
	}{
		Comment:  "Generated by `pnpm check third-party-notices` from the lockfiles and " + vendoredCreditsPath + ". Don't edit by hand. Full license texts: THIRD-PARTY-NOTICES.md",
		Vendored: make([]attributedPackage, 0, len(vendored)),
		Rust:     rust,
		Npm:      npm,
	}
	for _, credit := range vendored {
		payload.Vendored = append(payload.Vendored, credit.asPackage())
	}
	if rust == nil {
...
func renderNotices(rust rustCollection, npm []attributedPackage, vendored []vendoredCredit) []byte {
	var b strings.Builder

	b.WriteString("# Third-party notices\n\n")
	b.WriteString("Cmdr is built on open-source software. This file lists every third-party package, icon set, and other\n")
	b.WriteString("file that ships inside the app, with its license, and reproduces the license texts in full.\n\n")
	b.WriteString("Generated by `pnpm check third-party-notices` from `Cargo.lock`, `pnpm-lock.yaml`, and the hand-kept\n")
	b.WriteString("`" + vendoredCreditsPath + "`. Don't edit it by hand; edit that list or the generator at\n")
	b.WriteString("`scripts/check/checks/desktop-third-party-notices.go` instead.\n\n")

	fmt.Fprintf(&b, "- Rust crates: %d\n", len(rust.packages))
	fmt.Fprintf(&b, "- npm packages: %d\n", len(npm))
	fmt.Fprintf(&b, "- Icons and other files: %d\n", len(vendored))
	fmt.Fprintf(&b, "- Distinct license texts: %d\n\n", len(rust.texts))

	writeVendoredSection(&b, vendored)
