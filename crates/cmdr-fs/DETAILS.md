...
- **The runner (`runner.rs`).** The closed `Call` enum IS the verb list: `hdiutil create`, `attach -plist -nobrowse`,
  ...
  `unmount` and `unmountDisk` are what a DiskArbitration approval test drives, since both go through DA. A verb gets
  added with its first caller. Every call is SIGKILLed past 30 s and reaped. Output goes to anonymous temp files rather
