...
//! gate, so each holds `crate::test_read_pool_lock()` to serialize against
//! parallel tests and resets the gate itself.
//!
//! The live tick has its own file (`live_tests.rs`); it shares the fixtures below, which
//! `reclaim_tests` also reaches for.
...
pub(super) fn fake_backend() -> Arc<dyn VisionBackend> {
...
pub(super) fn config_with(always_folders: &[&str], excluded: &[&str]) -> NetworkEnrichConfig {
    NetworkEnrichConfig {
...
pub(super) fn seed_importance_full_pass(data_dir: &std::path::Path, rows: &[(&str, f64)]) {
...
pub(super) fn seed_media_row(data_dir: &std::path::Path, path: &str) {
