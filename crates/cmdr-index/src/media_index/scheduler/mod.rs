...
/// The media index's background worker: one pass at a time per volume, over the
/// images the file index found. The host holds it and kicks it; everything else
/// about a pass (pacing, coalescing, retry-when-idle) is decided in here.
...
impl MediaScheduler {
    ...
    pub fn new(data_dir: PathBuf, backend: Arc<dyn VisionBackend>) -> Self {
        ...
        Self::new_with_factory(data_dir, backend, factory, crate::NoopEventSink::shared())
    }
