...
- **`media_index Decision N` (bare `Decision N` here) means `DETAILS.md` § "Key decisions"**, the numbered founding
  log; ❌ never renumber it. A bare `plan M<n>` means a wiped resource plan, NOT the faces/captions milestones: resolve
  it in `DETAILS.md` § "What a bare "plan M<n>" in this subsystem means"; ❌ never add a new one.
