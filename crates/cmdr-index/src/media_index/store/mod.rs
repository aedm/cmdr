...
pub struct MediaStatusRow {
    /// The image this row describes, absolute.
    pub path: String,
    /// Its modification time when the row was written, half the staleness key.
    pub mtime: Option<u64>,
    /// Its byte size then, the other half.
    pub size: Option<u64>,
    /// What kind of media it was recognized as.
    pub media_kind: MediaKind,
    /// Where the Vision side got to: enriched, skipped, or failed.
    pub state: EnrichmentState,
    /// The Vision provenance stamp (OS build + engine + tag taxonomy + feature-print
    /// revision) the row was written under. A change re-runs the Vision side.
    pub engine_version: String,
    /// The CLIP provenance stamp (model id + OS version), empty when no model has
    /// embedded this row. Independent of `engine_version` on purpose, so each side
    /// re-runs on its own model change.
...
pub enum MediaStoreError {
    ...
    Io(std::io::Error),
    /// The on-disk schema version differs from this build's. Not a failure: `open`
    /// deletes and recreates the file fresh.
    SchemaMismatch {
