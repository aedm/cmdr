...
//! then everything else with removal-storm coalescing), and the live-path helper
//! `mark_pending_and_drain`. The rename pre-pass itself is `rename_detect.rs`;
//! shared primitives (`merge_fs_events`, `open_read_conn_with_retry`, the
//! flush-interval constants) live in the parent `event_loop` module.
...
pub(in crate::indexing) fn process_live_batch(
    ...
    let renamed_paths = super::rename_detect::detect_renames_by_inode(
        &mut other_events,
        space,
        conn,
        writer,
