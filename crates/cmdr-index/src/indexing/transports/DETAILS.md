...
- **The user turned this share ON** — the sticky `user_enabled` meta marker, read off a short-lived READ-ONLY connection
  ...
  choice stays on record, and this resume brings the share in when it reconnects. (`persisted_scan_completed` is the
  third arm, for indexes enabled before the marker existed. ❌ Never read it as the enable: it's absent all through a
  first index and all through every rescan.)
...
- **Emit-after-write ordering.** The inline pane-enrich reads sizes BEFORE the index write would land, so the write is
  sequenced FIRST and the writer emits `DirsUpdated` for the affected dir (`EmitDirUpdated`, which rides the same writer
  channel so it fires only after the upsert/delete commits). The existing FE refresh path (`DirsUpdated` →
...
- The rest matches SMB verbatim: enqueue on the volume's writer, index write before the `DirsUpdated` emit, reads off
  the `ReadPool`, buffer-during-scan (`SCAN_CHANGE_BUFFER`, 50,000, overflow ⇒ Stale) replayed after aggregation,
...
re-enable reconciles in place). Idempotent, so overlapping hooks are safe. All three go through
`Index::stop_removable_volume`, which acts ONLY for a `LocalExternal` index; SMB and MTP tear their indexes down through
their own disconnect paths, and stopping them here would fight that. It answers once the index has LET GO of the drive,
not once the stop was asked: a stop landing while the index is still starting, or while a scan start or another teardown
holds its manager, returns with the watcher still up (`../lifecycle/DETAILS.md` § "When a volume has been let go"). The
