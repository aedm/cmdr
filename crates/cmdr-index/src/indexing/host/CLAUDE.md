...
- `volumes.rs` — what's mounted (`MountIdentity`), where, what kind of storage, plus the SMB upgrade and MTP handle
  resolution: `VolumeProvider`, `MountFacts`, `NoVolumes`, and the test `FakeVolumeProvider`. App side:
