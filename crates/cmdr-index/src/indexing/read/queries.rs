...
pub fn get_volume_index_status(volume_id: &str) -> VolumeIndexStatus {
    ...
    let sweep = reconciler::sweep_record(volume_id);
    ...
    let next_sweep_due_at = sweep
        ...
        .map(|at| at + reconciler::SHALLOW_RESCAN_MIN_INTERVAL.as_secs());
...
pub fn get_status(volume_id: &str) -> Result<IndexStatusResponse, String> {
    ...
    match reg.get(volume_id).map(|i| &i.phase) {
        ...
        // a rescan began. No numbers yet, and nothing here may read a database.
        Some(IndexPhase::Detached { .. }) => Ok(IndexStatusResponse {
            initialized: true,
            scanning: true,
