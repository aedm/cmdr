...
//!
//! Which primitive reads a root, and how one root's walk ended, are `ground.rs`:
//! the local guarded walker versus the `Volume` trait, and the serial-reconcile
//! repair path for a frontier node that isn't virgin. Everything here is the same
//! whichever of those took a root.
