...
failure stays the caller's to see. It costs one extra parent listing, on the error path only. A phone whose session is
gone answers `VolumeError::DeviceDisconnected` (`volume/mapping.rs`), never `NotFound`, so it can't pass for an empty
destination (`volume/disconnected_test.rs`).
